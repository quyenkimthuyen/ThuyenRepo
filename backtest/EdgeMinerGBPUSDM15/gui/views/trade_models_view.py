...
def _render_models_overview(models: list[dict], active: dict | None):
  ...
  if selected_idx is None or not (0 <= selected_idx < len(rows)):
    st.caption("Tip: click 1 dòng → Đặt Active / Đổi tên / Xóa.")
  ...
  already_active = active_id == mid
  on_bridge = mid in {str(x) for x in (bridge_ids or []) if x}

  st.markdown(f"**Đang chọn:** {lab}")
  st.caption(
  ...
  with a1:
    if st.button(
      "Đã Active" if already_active else "Đặt Active",
      type="primary",
      disabled=already_active,
      use_container_width=True,
      key="tm_overview_set_active",
    ):
      updated = set_active_trade_model(mid)
      if updated:
        _apply_pending_model_pick(updated)
        st.toast(f"Active → {format_model_label(updated)}")
        st.rerun()
  with a2:
  ...
  with st.expander("Ghi chú đọc bảng", expanded=False):
    st.markdown(
      ...
      "- **Nguồn**: `OOS` = từ report remine/backtest; `Grid` = KPI lúc tạo từ Grid Search.\n"
      "- Sort mặc định theo **Total R** giảm dần — click header cột để sort lại.\n"
      "- **Đổi tên / Xóa** chỉ trên tab này (không còn nút trên thanh chọn phía trên)."
    )
