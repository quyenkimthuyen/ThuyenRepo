...
def _desk_ctx() -> dict:
  ...
  pair_label = "EUR/USD" if pair == "EUR" else ("GBP/USD" if pair == "GBP" else pair)
...
- Click dòng → **Đổi tên** · **Archive** · **Xóa cứng** (xác nhận)
...
"""
  ...
  st.markdown(
    f"""
### Compare Trade
So 1–N model trên cache M15 (`{d['cache']}`). Không EA. Kết quả = sim fills — **không** mở lệnh tài khoản.
...
4. Mỗi M15: EA `bar.json` → App decide → `decision.json` → BUY/SELL/FLAT → `fill.json` → `trades.json`.
