...
$SourceEaLive = Join-Path $RepoRoot "mt5\Experts\ForgeBridgeM15G23.mq5"
$SourceEaSim = Join-Path $RepoRoot "mt5\Experts\ForgeBridgeM15G23Sim.mq5"
$ProjectBridge = Join-Path $RepoRoot "mt5\bridge_m15g23"
$ProjectBridgeSim = Join-Path $RepoRoot "mt5\bridge_sim_m15g23"
$BridgeSubdirLive = "bridge_m15g23"
$BridgeSubdirSim = "bridge_sim_m15g23"
$EaNameLive = "ForgeBridgeM15G23"
$EaNameSim = "ForgeBridgeM15G23Sim"
$EaFolder = "EdgeMinerM15G23"
$EaMagicLive = 20261023
$EaMagicSim = 20262023
$IsBoth = ($Mode -eq "Both")
...
function New-GBPUSDM15Chart([string]$DataPath) {
  ...
  if ($templates.Count -eq 0) {
    Write-Host "No GBPUSD template anywhere - creating blank GBPUSD M15 chart."
  ...
  Write-Host "Created GBPUSD M15 chart in active profile: $targetPath"
...
function Select-AttachChart(
  ...
  if ($free.Count -gt 0) {
    Write-Host "Attach target: free GBPUSD M15 chart $($free[0].Name) for $wantedName"
    return $free[0]
  }
  ...
  Write-Host "No free GBPUSD M15 chart for $wantedName; creating a new chart (keep $otherName intact)."
  $created = @(New-GBPUSDM15Chart $DataPath)
  if ($created.Count -eq 0) {
    throw ("Failed to create GBPUSD M15 chart for $wantedName.")
  }
...
function Restart-BridgeService {
  ...
  $commandLine = (
    ...
    "--monitor-port 8995 --bridge-dir `"$ProjectBridge`""
  )
...
if ($Attach) {
  if ($IsBoth) {
    ...
    Write-Step "Attach $EaName HISTORY_FEED to GBPUSD M15"
    ...
    Write-Step "Attach $EaName Live to GBPUSD M15"
