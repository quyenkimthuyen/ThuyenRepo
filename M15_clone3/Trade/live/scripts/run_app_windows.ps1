...
function Find-FreeAppPort {
  ...
  foreach ($p in 9401, 9402, 9403, 9404, 9405) {
...
function Invoke-HealAfterMove {
  # Rewrite absolute paths if LiveCheck was moved/copied to a new folder.
  $liveCheck = (Resolve-Path (Join-Path $RepoRoot "..\..")).Path
...
function Start-App {
  ...
  if ($Port -ne 9401) {
    Write-Host "Using fallback port $Port (9401 blocked by stale/ghost socket)."
  }
