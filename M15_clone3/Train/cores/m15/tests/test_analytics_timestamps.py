...
from analytics import trades_json_to_df, weekly_breakdown, daily_breakdown
from gui.bridge_model_monitor import live_trades_to_analytics_df, build_weekly_series_figure, build_daily_series_figure
...
def test_daily_breakdown_calendar_day():
  df = trades_json_to_df(
    [
      {"entry": "2026.08.24 08:00", "r": 1.0},
      {"entry": "2026.08.24 14:15", "r": -0.5},
      {"entry": "2026.08.26 10:15", "r": 2.0},
    ]
  )
  d = daily_breakdown(df)
  assert list(d["day"]) == ["2026-08-24", "2026-08-26"]
  assert list(d["n_trades"]) == [2, 1]
  assert list(d["total_r"]) == [0.5, 2.0]
  assert list(d["cum_r"]) == [0.5, 2.5]
...
def test_daily_series_figure():
  d = daily_breakdown(
    trades_json_to_df(
      [
        {"entry": "2026.08.24 08:00", "r": 1.0},
        {"entry": "2026.08.24 14:00", "r": -0.5},
        {"entry": "2026.08.26 07:00", "r": 2.0},
      ]
    )
  )
  fig = build_daily_series_figure(d, title="Ngày", series_name="Live Auto")
  assert fig is not None
  assert len(fig.data) == 2
  assert list(fig.data[0].x) == ["2026-08-24", "2026-08-26"]
