...
def test_roc_5_ignores_future_bars_and_matches_prefix_series():
  """Grid FeatureMatrix is the full file; roc_5 must still be bar-causal like live."""
  n = 400
  probe = 200
  baseline = _frame(periods=n)
  shocked = baseline.copy()
  shocked.iloc[300:, :] += 0.05

  ts = baseline.index[probe]
  live = FeatureMatrix(baseline.iloc[: probe + 1])
  grid = FeatureMatrix(shocked)
  i_live = int(live.index.get_loc(ts))
  i_grid = int(grid.index.get_loc(ts))

  np.testing.assert_allclose(
    live.get("roc_5")[i_live], grid.get("roc_5")[i_grid], rtol=1e-9, atol=1e-9,
  )
  for name in ("rsi", "zscore_20", "bb_pos", "session_vwap_dist", "htf_trend"):
    a = live.get(name)[i_live]
    b = grid.get(name)[i_grid]
    if np.isnan(a) and np.isnan(b):
      continue
    np.testing.assert_allclose(a, b, rtol=1e-9, atol=1e-9, err_msg=name)
