...
def test_causal_oos_end_stops_at_closed_bar():
  from mt5_bridge.engine import causal_oos_end

  # Week scan end=100, deciding bar 40 → exclusive end 41
  assert causal_oos_end(100, 40) == 41
  assert causal_oos_end(41, 40) == 41
  # Never extend past the caller's exclusive end
  assert causal_oos_end(40, 40) == 40
  assert causal_oos_end(10, 40) == 10


def test_compare_project_levels_does_not_peek_next_open():
  """Preloaded Compare parquet has later bars; planned entry must use this close."""
  import numpy as np
  ...
  n = 3
  fm = type("FM", (), {})()
  fm.index = pd.date_range("2026-01-05 10:00", periods=n, freq="15min")
  fm.n = n
  fm.open = np.array([1.1000, 1.1080, 1.1090])
  fm.high = np.array([1.1005, 1.1085, 1.1095])
  fm.low = np.array([1.0995, 1.1075, 1.1085])
  fm.close = np.array([1.1002, 1.1078, 1.1088])
  fm.atr = np.full(n, 0.001)
  fm.hours = fm.index.hour.to_numpy()
  fm.spread_points = np.zeros(n)
  strat = MinedStrategy(atr_mult_sl=1.0, rr_ratio=2.0)
  peeked = _project_signal_levels(fm, strat, 0, 1, spread_pips=0.0, slippage_pips=0.0)
  causal = _project_signal_levels(
    fm, strat, 0, 1, spread_pips=0.0, slippage_pips=0.0, as_of_idx=0,
  )
  assert peeked is not None and causal is not None
  assert peeked["entry_px"] == pytest.approx(1.1080)
  assert causal["entry_px"] == pytest.approx(1.1002)
  assert causal["entry_px"] != peeked["entry_px"]
