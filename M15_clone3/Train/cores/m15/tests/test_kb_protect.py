...
def test_ensure_profile_learned_continues_protected_without_reset(kb_tmp, monkeypatch):
  kbp = kb_tmp["kbp"]
  _touch_profile(kbp, "era_lock")
  kbp.protect_profile("era_lock", reason="tm:x")
  kbp.register_profile("era_lock", "era_lock", "2025-07-01", "2025-12-31", 2)
  ...
  def fake_learn(**kwargs):
    ...
    return {"epochs": kwargs["epochs"], "kb_profile": kwargs["kb_profile"]}

  monkeypatch.setattr("gui.services.execute_learning", fake_learn)
  out = ensure_profile_learned(
    {
      "kb_profile": "era_lock",
      "kb_name": "lock",
  ...
  assert not out.get("protected")
  assert called.get("reset_kb") is False
  assert called.get("epochs") == 4
  assert called.get("kb_profile") == "era_lock"
