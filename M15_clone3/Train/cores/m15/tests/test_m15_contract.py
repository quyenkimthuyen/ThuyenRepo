...
def test_daily_cap_is_first_come_not_best_later_score():
  """Live cannot skip a morning signal because afternoon will score higher."""
  fm = _SignalMatrix()

  class _LaterBetterML:
    def __init__(self, n: int):
      self._prob_long = np.linspace(0.45, 0.95, n)
      self._prob_short = np.full(n, 0.1)

    def refresh_for_fm(self, _fm):
      return None
  ...
  strategy = MinedStrategy(
    ...
    session_filter=False,
    ml_scorer=_LaterBetterML(fm.n),
  )
  signals = generate_signals_mined(fm, strategy)
  assert list(np.flatnonzero(signals)) == [0, 8]
