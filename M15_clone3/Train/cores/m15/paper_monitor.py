...
def _project_signal_levels(
  ...
  *,
  ...
  Live / Compare: at the closed signal bar the next open is unknown. Pass
  ``as_of_idx=bar_idx`` so a preloaded full file cannot peek ``open[i+1]``.
  Fill still happens at the next bar open (EA / PaperBook) and rebases.
  SL = ATR×mult + 1 spread (Ask/Bid) để live không chết trong spread.
  """
  last_known = fm.n - 1 if as_of_idx is None else min(int(as_of_idx), fm.n - 1)
  if bar_idx < 0 or bar_idx > last_known:
    return None
  entry_idx = bar_idx + 1
  av = fm.atr[bar_idx]
  if pd.isna(av) or av <= 0:
    return None
  last_pts = 0.0
  peek_next = entry_idx <= last_known
  look = entry_idx if peek_next else bar_idx
...
def get_monitor_state(
  ...
  from strategy_miner import apply_oos_exit_overlay
  ...
  week_trades, open_position = backtest_mined(
    fm, bt_strat, signals, oos_start, oos_end,
  ...
  return {
    ...
    "strategy": {
      "name": bt_strat.name,
      "exit_mode": bt_strat.exit_mode,
      "rr": bt_strat.rr_ratio,
      "atr_mult_sl": bt_strat.atr_mult_sl,
      "ml_prob_min": bt_strat.ml_prob_min,
      "max_trades_per_day": bt_strat.max_trades_per_day,
      "max_hold_bars": bt_strat.max_hold_bars,
    },
