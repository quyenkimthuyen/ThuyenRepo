...
Replay is bar-causal: PaperBook sees one OHLC at a time; ``decide_for_bar``
does not scan bars after the closed bar (full cache is only for remine lookback).
"""
...
MAX_MODELS = 5
MAX_ADDITIONAL_SPREAD_PIPS = 5.0
...
def summarize_run(run: dict, *, is_latest: bool = False) -> dict[str, Any]:
  ...
  return {
    ...
    "additional_spread_pips": run.get("additional_spread_pips") or 0.0,
...
def _bar_dict(ts: pd.Timestamp, row: pd.Series, extra_spread_pts: int = 0) -> dict:
  ...
  payload["spread_points"] = _row_spread_points(row) + max(0, int(extra_spread_pts or 0))
  return payload
...
def clamp_additional_spread_pips(value) -> float:
  """Non-negative extra pips on top of quote/model spread. 0 = no stress."""
  try:
  ...
  if v < 0:
    return 0.0
  return round(min(v, MAX_ADDITIONAL_SPREAD_PIPS), 4)


def additional_spread_points(additional_spread_pips: float) -> int:
  from execution import pips_to_points
  return pips_to_points(clamp_additional_spread_pips(additional_spread_pips))


def bump_spread_points(
  df: pd.DataFrame,
  extra_pts: int,
  index: pd.Index | None = None,
) -> pd.DataFrame:
  """Copy of ``df`` with ``SpreadPoints`` raised on ``index`` (or all rows)."""
  extra = int(extra_pts or 0)
  if extra <= 0 or df is None or df.empty:
    return df
  out = df.copy()
  if "SpreadPoints" not in out.columns:
    out["SpreadPoints"] = 0.0
  else:
    out["SpreadPoints"] = pd.to_numeric(out["SpreadPoints"], errors="coerce").fillna(0.0)
  if index is None:
    out["SpreadPoints"] = out["SpreadPoints"] + extra
  else:
    common = out.index.intersection(index)
    if len(common):
      out.loc[common, "SpreadPoints"] = out.loc[common, "SpreadPoints"] + extra
  return out
...
def run_compare(
  *,
  ...
  extra_pips = clamp_additional_spread_pips(additional_spread_pips)
  extra_pts = additional_spread_points(extra_pips)
  ...
  run: dict[str, Any] = {
    ...
    "additional_spread_pips": extra_pips,
    "additional_spread_points": extra_pts,
    "bars_done": 0,
  ...
  for mid in ids:
    ...
    # Prefill full history for remine lookback (avoid per-bar parquet writes).
    # decide_for_bar still scans only through the closed bar — no future peek.
    working = full.copy()
    if extra_pts:
      working = bump_spread_points(working, extra_pts, replay.index)
    eng._df = working
    if extra_pips:
      base = float(eng._params.get("spread_pips") or 0.0)
      eng._params = {**eng._params, "spread_pips": round(base + extra_pips, 4)}
    engines[mid] = eng
  ...
  try:
    for i, (ts, row) in enumerate(replay.iterrows(), start=1):
      ...
      bar = _bar_dict(ts, row, extra_pts)
      bar_time = bar["time"]
      fill_pts = int(bar["spread_points"])
