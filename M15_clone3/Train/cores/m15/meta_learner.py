...
from strategy_miner import (
  ...
  apply_breakthrough_filters, _exec_cost_kwargs, _is_reliable_feature_set,
  apply_feature_reliability_weights,
)

_DETERMINISM_LOCK = threading.RLock()


def _apply_kb_and_reliability(strat, kb: KnowledgeBase, space, *, as_of=None, controlled: bool = False):
  strat = apply_kb_rule_weights(strat, kb, as_of=as_of)
  if _is_reliable_feature_set(space if controlled else None):
    strat.long_rules = apply_feature_reliability_weights(strat.long_rules)
    strat.short_rules = apply_feature_reliability_weights(strat.short_rules)
  return strat
...
def _mine_strategy_learning_impl(
  ...
  if kb.genomes:
    ...
    top_n = int(getattr(space, "kb_seed_top_n", 0) or 0) if controlled else 0
    if top_n <= 0:
      top_n = 3
    candidates.extend(kb.top_genomes(top_n))
  ...
  for g in candidates:
    ...
    strat.ml_scorer = ml
    strat = _apply_kb_and_reliability(
      strat, kb, space, as_of=as_of, controlled=controlled,
    )
  ...
  # hit80 must NOT always remine: forcing fresh mine OOS-collapsed WR to 49%.
  if best_score < 50 or best is None:
    ...
    if fresh:
      fresh.ml_scorer = ml
      fresh = _apply_kb_and_reliability(
        fresh, kb, space, as_of=as_of, controlled=controlled,
      )
  ...
  if best is not None:
    ...
    for _ in range(6):
      ...
      mutant.ml_scorer = ml
      mutant = _apply_kb_and_reliability(
        mutant, kb, space, as_of=as_of, controlled=controlled,
      )
