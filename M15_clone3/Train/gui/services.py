  ...
  if feature_profile is None or mining_search_space is None or kb_pin_path is None:
    try:
      ...
      if feature_profile is None:
        feature_profile = mp.get("feature_profile") or DEFAULT_FEATURE_PROFILE
      ...
      feature_profile = feature_profile or DEFAULT_FEATURE_PROFILE
...
def execute_learning(
  ...
  if is_profile_protected(kb_profile) and reset_kb:
    raise ValueError(
  ...
  have = len(kb.epoch_history)
  start = 1 if reset_kb else (have + 1)
  all_epoch_results = []
  last_result = None
  if start > epochs:
    register_profile(
      kb_profile, kb_name or kb_profile,
      str(df.index[0].date()), str(df.index[-1].date()), have,
    )
    return {
      "epochs": have,
      "skipped": True,
  ...
  register_profile(
    kb_profile, kb_name or kb_profile,
    str(df.index[0].date()), str(df.index[-1].date()), len(kb.epoch_history),
  )

  report = {
    "epochs": len(kb.epoch_history),
    "kb_profile": kb_profile,
    "trained_from": str(df.index[0].date()),
    "trained_to": str(df.index[-1].date()),
...
def get_paper_monitor(
  ...
  feature_profile = feature_profile or model_params.get("feature_profile") or DEFAULT_FEATURE_PROFILE
  if mining_search_space is None:
