...
from gui.desk_ui import bars_per_day, chart_bars_presets, tf_label
...
def _mt5_cache_range() -> tuple[date | None, date | None]:
  """Available broker dates in MT5 cache (Compare source)."""
  try:
...
def _compare_history_label(summary: dict) -> str:
  ...
  try:
    extra = float(summary.get("additional_spread_pips") or 0.0)
  except (TypeError, ValueError):
    extra = 0.0
  extra_txt = f" · +{extra:g} pip" if extra > 0 else ""
  return (
    f"{when} · `{rid}` · {d0}→{d1}{extra_txt} · {n} model · best {best_txt} · {status}{tag}"
  )
...
def render():
  ...
  st.caption(
    ...
    "**Additional spread** cộng thêm pip vào quote nến (stress live rộng hơn lúc train)."
  )

  cache_from, cache_to = _mt5_cache_range()
  if cache_from and cache_to:
    st.info(
      f"MT5 {tf_label()} cache: **{cache_from} → {cache_to}**. "
      "Compare chỉ chạy trong khoảng này "
      "(đồng bộ thêm history ở MT5 Bridge nếu cần năm cũ hơn)."
    )
  else:
    st.warning(
      f"Chưa có MT5 {tf_label()} cache — Start Bridge / sync history trước khi Compare."
    )
  ...
  from analytics import equity_series, monthly_breakdown, weekly_breakdown, daily_breakdown
  ...
  restore_widget(
    "cmp_add_spread", 0.0,
    preference_key="compare.additional_spread_pips",
    decode=lambda v: float(v),
  )
  restore_widget(
    "cmp_models", labels[:2],
  ...
  with c4:
    st.number_input(
      ...
      key="cmp_add_spread",
      disabled=running or blocked,
      on_change=preference_callback("cmp_add_spread", "compare.additional_spread_pips"),
      help=(
        "Cộng thêm vào spread quote của từng nến khi fill (BUY Ask / SELL Bid) "
        "và SL buffer. 0 = đúng lịch sử. Thử 0.5–2.0 để xem TM nào chịu spread live xấu hơn."
      ),
    )
  ...
  extra_spread = float(st.session_state.get("cmp_add_spread") or 0.0)
  ...
  if start:
    if d_to < d_from:
      ...
      st.error(
        f"Khoảng {d_from} → {d_to} nằm ngoài MT5 {tf_label()} cache "
      ...
      set_preference("compare.additional_spread_pips", extra_spread)
      ...
      try:
        start_job(
          "compare_trade",
          {
            ...
            "risk_pct": risk,
            "additional_spread_pips": extra_spread,
          },
          label="Compare Trade",
        )
  ...
  try:
    extra_run = float(run.get("additional_spread_pips") or 0.0)
  except (TypeError, ValueError):
    extra_run = 0.0
  extra_cap = (
    f" · additional spread **+{extra_run:g} pip** (quote nến + extra)"
    if extra_run > 0 else " · additional spread **0** (đúng quote lịch sử)"
  )
  st.caption(
  ...
  for mid in run.get("model_ids") or []:
    ...
    if not df.empty:
      ...
      weekly_by_model[label] = weekly_breakdown(df)
      daily_by_model[label] = daily_breakdown(df)
  ...
  tab_m, tab_w, tab_d = st.tabs(["R theo tháng", "R theo tuần", "R theo ngày"])
  with tab_m:
    ...
    if mo_fig is not None:
      ...
      st.caption("Chưa đủ lệnh đóng để vẽ R theo tháng.")
  with tab_w:
    wk_fig = build_multi_model_weekly_figure(weekly_by_model, title="R theo tuần · Compare Trade")
    if wk_fig is not None:
      st.plotly_chart(wk_fig, use_container_width=True)
      st.caption("Gom theo tuần bắt đầu **thứ Hai** (cùng mốc remine).")
    else:
      st.caption("Chưa đủ lệnh đóng để vẽ R theo tuần.")
  with tab_d:
    dy_fig = build_multi_model_daily_figure(daily_by_model, title="R theo ngày · Compare Trade")
    if dy_fig is not None:
      st.plotly_chart(dy_fig, use_container_width=True)
      st.caption("Chỉ hiện **ngày có lệnh**. Bar = R ngày đó, đường dưới = R tích lũy.")
    else:
      st.caption("Chưa đủ lệnh đóng để vẽ R theo ngày.")
  ...
  if chart_labels:
    ...
    max_bars = chart_bars.get(range_label, bars_per_day() * 7)
    ...
    if server_ok:
      ...
      price_fig = build_multi_model_price_figure(
        ohlc,
        filtered,
        title=f"Giá {tf_label()} · {from_txt} → {to_txt} · {range_label} ({n_bars} bar)",
        show_connectors=bool(show_lines),
      )
