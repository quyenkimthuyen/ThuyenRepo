...
def _get_chart_figure(state: dict) -> go.Figure | None:
  ...
  try:
    ...
    fig = _tradingview_chart(
      ...
      title=f"EUR/USD M5 · Tuần {state.get('week_start', '')}",
    )
  except Exception:
    return None
...
def _bars_for_period(date_from: date | None, date_to: date | None) -> int:
  ...
  return max(96, min(5000, days * 288))
...
def _render_chart_panel(
  ...
  *,
  ...
  if server_ready:
    ...
    st.caption(
      f"Chart Paper · giai đoạn **{label}** · ~{max_bars} nến M5 · "
  ...
  try:
    window = get_ohlc_window_cached(chart_from, chart_to)
    chart_title = f"EUR/USD M5 · {chart_from} → {chart_to}"
    fig = _tradingview_chart(
...
def _render_legacy_paper_desk_removed():
  ...
  with st.expander("Cấu hình service", expanded=not status.get("running")):
    st.caption(
      ...
      "service chỉ tính lại khi có nến M5 mới hoặc cấu hình/model thay đổi. "
