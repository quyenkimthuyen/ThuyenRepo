...
def status_banner(report: dict | None, kb: dict, data_meta: dict):
  if report:
    ...
    st.caption(
      ...
      f"Last sync: {str(data_meta.get('synced_at') or 'n/a')[:10]} · "
...
def warn_long_bias(pct_long: float):
  if pct_long >= 85:
    try:
      from gui.desk_ui import pair_label
      pair = pair_label()
    except Exception:
      pair = "cặp này"
    st.warning(
      f"**LONG bias {pct_long:.0f}%** — strategy có thể chỉ hoạt động khi {pair} tăng. "
