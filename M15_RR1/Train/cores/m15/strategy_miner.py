...
class MinedStrategy:
  ...
  # 0 = after partial, park remainder SL at +0.1R. >0 = trail remainder (floor +0.1R).
  partial_runner_trail_r: float = 0.0
...
class MiningSearchSpace:
  ...
  # legacy | expectancy_frontier | elite_frontier | flow_frontier | hit80
  # flow_frontier: hard floor ~5 trades/week + WR≥50 (volume book, not sniper).
  # hit80: WR≥80 hunt — preferred gate WR≥80% (Total R>80 is Grid/OOS, not 8w train).
  ...
  force_side: str = "both"
  # 0 = default 3. hit80 uses more KB elites so setup DNA is not just top-3 stale fitness.
  kb_seed_top_n: int = 0
  # full = mine the whole zoo. reliable = drop noisy indicators and boost
  # RSI / VWAP / EMA / sweep / rejection / confluence (see FEATURE_RELIABILITY).
  feature_set: str = "full"
  # 0 = default 0.35 PA confluence add-on. >0 overrides in generate_signals.
...
def constrain_strategy_to_space(
  ...
  allowed = allowed_rule_features(space)
  if allowed is not None:
    strat.long_rules = [r for r in strat.long_rules if r.feature in allowed]
    strat.short_rules = [r for r in strat.short_rules if r.feature in allowed]
    strat.long_rules, strat.short_rules = _ensure_seed_rules(
      strat.long_rules, strat.short_rules,
    )
  if str(getattr(space, "feature_set", "full") or "full").lower() == "reliable":
    strat.htf_align_boost = 1.18
    strat.htf_counter_dampen = 0.82
    bonus = float(getattr(space, "confluence_bonus", 0.0) or 0.0)
    strat.confluence_bonus_scale = bonus if bonus > 0 else 0.50
  ...
  if str(getattr(space, "exit_mode_lock", "") or "").strip().lower() == "partial":
    pct = float(getattr(space, "partial_pct", 0.0) or 0.0)
    at = float(getattr(space, "partial_at_r", 0.0) or 0.0)
    runner = float(getattr(space, "partial_runner_trail_r", 0.0) or 0.0)
    if pct > 0:
      strat.partial_pct = pct
    if at > 0:
      strat.partial_at_r = at
    if runner > 0:
      strat.partial_runner_trail_r = runner
  return strat
...
def apply_oos_exit_overlay(
  ...
  if mode == "partial":
    ...
    runner = float(getattr(space, "partial_runner_trail_r", 0.0) or 0.0)
    if runner > 0:
      kw["partial_runner_trail_r"] = runner
...
# Reliability set: keep trend / RSI / VWAP / ATR / quality PA. Drop MACD/ROC
# (overlap + noisy), engulf/range/squeeze/displacement (KB WR collapsed),
# candle wicks, and full-session binaries (hour window already gates time).
RELIABLE_CONTINUOUS = [
  "rsi", "adx", "bb_pos", "bb_width_pct", "atr_pct",
  "price_vs_ema21", "price_vs_ema50", "ema_slope_8", "ema_slope_21",
  "session_vwap_dist", "swing_strength",
]
RELIABLE_BINARY_LONG = [
  "sweep_low_fade", "pullback_long", "ema_stack_bull",
  "rejection_bull", "structure_break_up", "confluence_long",
]
RELIABLE_BINARY_SHORT = [
  "sweep_high_fade", "pullback_short", "ema_stack_bear",
  "rejection_bear", "structure_break_dn", "confluence_short",
]
RELIABLE_SESSION_REGIME = [
  "overlap_session", "london_open", "regime_trending", "regime_high_vol",
]
FEATURE_RELIABILITY = {
  "rsi": 1.40,
  "session_vwap_dist": 1.40,
  "confluence_long": 1.35,
  "confluence_short": 1.35,
  "rejection_bull": 1.30,
  "rejection_bear": 1.30,
  "sweep_low_fade": 1.25,
  "sweep_high_fade": 1.25,
  "ema_stack_bull": 1.25,
  "ema_stack_bear": 1.25,
  "ema_slope_8": 1.20,
  "ema_slope_21": 1.20,
  ...
  "adx": 1.20,
  "atr_pct": 1.10,
  "structure_break_up": 1.15,
  "structure_break_dn": 1.15,
  "bb_pos": 1.10,
  "bb_width_pct": 1.05,
  "swing_strength": 1.10,
  "overlap_session": 1.05,
  "london_open": 1.05,
  "regime_trending": 1.10,
  "regime_high_vol": 1.05,
  "roc_5": 0.40,
  "macd_hist": 0.45,
  "macd_cross_up": 0.55,
  "macd_cross_dn": 0.55,
  "engulf_bull": 0.35,
  "engulf_bear": 0.35,
  "squeeze_break_up": 0.45,
  "squeeze_break_dn": 0.45,
  "range_buy": 0.35,
  "range_sell": 0.35,
  "displacement_bull": 0.45,
  "displacement_bear": 0.45,
  "body_ratio": 0.65,
  "lower_wick_ratio": 0.70,
...
def _is_reliable_feature_set(space: MiningSearchSpace | None) -> bool:
  return str(getattr(space, "feature_set", "full") or "full").lower() == "reliable"


def feature_lists_for_space(
  space: MiningSearchSpace | None,
) -> tuple[list[str], list[str], list[str], list[str]]:
  """Continuous, binary long, binary short, optional session/regime extras."""
  if _is_reliable_feature_set(space):
    extra = (
      list(RELIABLE_SESSION_REGIME)
      if space is not None and space.include_session_regime_rules
      else []
    )
    return (
      list(RELIABLE_CONTINUOUS),
      list(RELIABLE_BINARY_LONG),
      list(RELIABLE_BINARY_SHORT),
      extra,
    )
  extra = list(SESSION_REGIME_BINARY) if (
    space is not None and space.include_session_regime_rules
  ) else []
  return (
...
def allowed_rule_features(space: MiningSearchSpace | None) -> set[str] | None:
  """None = no prune. Otherwise only these features may stay on a genome."""
  if not _is_reliable_feature_set(space):
    return None
  cont, bl, bs, extra = feature_lists_for_space(space)
  return set(cont) | set(bl) | set(bs) | set(extra)


def apply_feature_reliability_weights(rules: list[Rule]) -> list[Rule]:
  """Re-score mined/KB rules: boost reliable indicators, demote noisy ones."""
  out: list[Rule] = []
  for r in rules:
    boost = float(FEATURE_RELIABILITY.get(r.feature, 1.0))
    out.append(Rule(r.feature, r.direction, r.op, r.threshold, r.weight * boost))
  return out


def _ensure_seed_rules(
  long_rules: list[Rule], short_rules: list[Rule],
) -> tuple[list[Rule], list[Rule]]:
  long_rules = list(long_rules)
  short_rules = list(short_rules)
...
def _exit_modes_for_space(space: MiningSearchSpace) -> list[tuple[str, dict]]:
  ...
  runner = float(getattr(space, "partial_runner_trail_r", 0.0) or 0.0)
  if runner > 0:
    partial_kw["partial_runner_trail_r"] = runner
  if lock == "hybrid":
...
def _pa_confluence_bonus(fm, i: int, direction: int, strat=None) -> float:
  ...
  if np.isnan(v):
    return 0.0
  scale = float(getattr(strat, "confluence_bonus_scale", 0.35) or 0.35)
  return scale * v
...
def generate_signals_mined(
  ...
  for i in range(max(start_idx, fm.warmup), stop):
    ...
    combined_l = ls * (0.5 + ml_l) * _htf_bias(fm, i, 1, strat) + _pa_confluence_bonus(fm, i, 1, strat)
    combined_s = ss * (0.5 + ml_s) * _htf_bias(fm, i, -1, strat) + _pa_confluence_bonus(fm, i, -1, strat)
  ...
  if candidates and strat.max_trades_per_day > 0:
    ...
    for items in day_buckets.values():
      # First-come like live: a morning fill cannot wait to see a better
      # afternoon score. Ranking by score peeks later bars of the same day.
...
def explain_bar_gates(fm, strat, i: int) -> dict:
  ...
  def _side(direction: int) -> dict:
    ...
    combined = (
      ...
      + _pa_confluence_bonus(fm, i, direction, strat)
    )
    other_combined = (
...
def backtest_mined(
  ...
  while i < end_idx - 1:
    ...
    if in_trade:
      ...
      runner_tr = float(getattr(strat, "partial_runner_trail_r", 0.0) or 0.0)
      if strat.exit_mode == "partial" and partial_done and runner_tr > 0:
        if direction == 1:
          sl = max(sl, bid_h - risk * runner_tr)
        else:
          sl = min(sl, ask_l + risk * runner_tr)
...
def score_strategy_metrics(
  ...
  # hit80: need ~3–5 tpw so 6m OOS at WR80 can print Total R>80. Never prefer
  # 1 tpw snipers (label_rr=2.2 starved EUR). Do not force 5+ (that printed WR47).
  if hit80:
    if tpw < 2.5:
      freq_score = -50.0 - (2.5 - tpw) * 40.0
    elif tpw > 8.0:
      freq_score = 25.0 - (tpw - 8.0) * 14.0
    else:
      freq_score = 80.0 + min(tpw - 2.5, 3.0) * 8.0
  elif flow:
    if tpw < 5.0:
  ...
  if hit80:
    s = (
      wr * 480.0
      + min(tr, 120.0) * 2.8
      + min(float(m["n_trades"]), 140.0) * 1.5
      + freq_score
    )
    if wr >= 0.80:
      s += 260.0
...
def _passes_best_gate(
  ...
  *,
  ...
  if tr <= 0:
    return False
  if selection_mode == "hit80":
    n = float(metrics.get("n_trades") or 0)
    # Sanity n on the train window, not a weekly cap (2.2R labels starved OOS).
    return wr >= 0.80 and tr > 0 and n >= 16
  ...
  for rr in space.rr_ratios:
    for atr_m in space.atr_multipliers:
      for max_hold in space.max_hold_bars:
        ...
        cont, bin_long, bin_short, extra_binary = feature_lists_for_space(space)
        for feat in cont:
        ...
        long_rules.extend(_mine_binary_rules(
          fm, bin_long + extra_binary, "long", long_wins,
          train_start, train_end, space.min_binary_samples,
        ))
        short_rules.extend(_mine_binary_rules(
          fm, bin_short + extra_binary, "short", short_wins,
          train_start, train_end, space.min_binary_samples,
        ))
        # Rank slots by lift × reliability prior; keep raw lift weights
        # (KB + apply_feature_reliability_weights score once before eval).
        long_rules = sorted(
          long_rules,
          key=lambda r: r.weight * FEATURE_RELIABILITY.get(r.feature, 1.0),
          reverse=True,
        )[:6]
        short_rules = sorted(
          short_rules,
        ...
        long_rules, short_rules = _ensure_seed_rules(long_rules, short_rules)
                  ...
                  for min_match in space.min_rules_matches:
                    for ml_thr in space.ml_probability_thresholds:
                      ...
                      if space.selection_mode in (
                        ...
                        if space.selection_mode in ("flow_frontier", "hit80"):
                          # Frequency / WR≥80 need the full train window; val is ~2–3 weeks.
                          gate_m = comb
                          gate_weeks = weeks
                        else:
