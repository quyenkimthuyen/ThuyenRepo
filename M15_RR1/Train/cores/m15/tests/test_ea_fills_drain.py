...
def test_reconcile_awaits_deal_when_ea_flat(tmp_path: Path):
  from mt5_bridge.protocol import atomic_write_json, connection_path, deals_path
  ...
  assert "awaiting_mt5_deal" in row.get("interventions", [])


def test_reconcile_force_close_when_user_confirms_flat(tmp_path: Path):
  from mt5_bridge.protocol import atomic_write_json, connection_path

  save_trades([_open_row()], tmp_path)
  atomic_write_json(connection_path(tmp_path), {"positions": 0, "connected": True})
  n = reconcile_ghost_opens_if_ea_flat(tmp_path, require_fresh_heartbeat=False)
  assert n == 1
