...
def _mt5_cache_range() -> tuple[date | None, date | None]:
  """Available broker dates in MT5 M5 cache (Compare source)."""
  try:
...
def render():
  ...
  if cache_from and cache_to:
    st.info(
      f"MT5 M5 cache: **{cache_from} → {cache_to}**. "
      "Compare chỉ chạy trong khoảng này "
      "(đồng bộ thêm history ở MT5 Bridge nếu cần năm cũ hơn)."
    )
  else:
    st.warning(
      "Chưa có MT5 M5 cache — Start Bridge / sync history trước khi Compare."
    )
  ...
  if start:
    if d_to < d_from:
      ...
      st.error(
        f"Khoảng {d_from} → {d_to} nằm ngoài MT5 M5 cache "
  ...
  if chart_labels:
    ...
    # M5 ≈ 288 bars/day — giống Simulate
    chart_bars = {
      "1 ngày": 96,
      "1 tuần": 672,
      "1 tháng": 2880,
    ...
    if server_ok:
      ...
      price_fig = build_multi_model_price_figure(
        ohlc,
        filtered,
        title=f"Giá M5 · {from_txt} → {to_txt} · {range_label} ({n_bars} bar)",
        show_connectors=bool(show_lines),
      )
