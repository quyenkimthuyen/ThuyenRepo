...
Task name is unique per clone folder (name + path hash) so copying the tree
to another directory does not steal another copy's Scheduled Task. After a
move, ``ensure_autostart_after_move`` rebinds the task to this copy's scripts.
"""
...
LEGACY_TASK_NAME = "EdgeMinerLive2Boot"
# Back-compat alias: callers/tests that still read TASK_NAME get the live name.
...
def clone_root() -> Path:
  """App clone root: ``<clone>/Trade/live`` → ``<clone>``."""
  return LIVE_ROOT.resolve().parent.parent


def clone_task_suffix(root: Path | None = None) -> str:
  """Stable per-folder id: ``M15_clone2-a1b2c3d4``."""
...
def task_name() -> str:
  return f"EdgeMinerLive2Boot-{clone_task_suffix()}"
...
def _stdout_has_installed(out: dict[str, Any]) -> bool:
  text = (out.get("stdout") or "").upper()
  return "INSTALLED" in text or "ALREADY_OK" in text
...
def task_status() -> dict[str, Any]:
  ...
  installed = _stdout_has_installed(out)
  return {
    "windows": True,
...
def enable_autostart(
  *,
  ...
  if out.get("ok"):
    prefs = save_prefs(enabled=True, task_name=task_name(), **draft)
...
def ensure_autostart_after_move() -> dict[str, Any]:
  """Rebind Scheduled Task to this copy if autostart is enabled.

  Safe to call on every Live Start / heal. No-op when disabled, not Windows,
  or the task already points at this folder's boot script.
  """
  if str(os.environ.get("LIVE_SKIP_AUTOSTART") or "").strip().lower() in (
    "1", "true", "yes", "on",
  ):
  ...
  if not bool(prefs.get("enabled")):
    return {"ok": True, "skipped": True, "reason": "not_enabled", "task_name": task_name()}
  if not is_windows():
    return {"ok": True, "skipped": True, "reason": "not_windows", "task_name": task_name()}
  delay = int(prefs.get("delay_sec") or 45)
  app_port = int(prefs.get("port") or LIVE_APP_PORT)
  out = _run_ps([
    "-Action", "Ensure",
    "-DelaySec", str(delay),
    "-Port", str(app_port),
  ], timeout_sec=45.0)
  ok = bool(out.get("ok")) and _stdout_has_installed(out)
  if ok:
    save_prefs(enabled=True, task_name=task_name())
  return {
    "ok": ok,
    "skipped": False,
    "task_name": task_name(),
    "task": out,
    "message": (out.get("stdout") or out.get("stderr") or out.get("reason") or "").strip(),
  }
