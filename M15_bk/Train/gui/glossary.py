...
HELP = {
  "oos": (
    "Giai đoạn **kiểm chứng (OOS)** — chỉ mô phỏng lệnh, không dùng để tối ưu. "
    "Grid không chạy combo nếu thời gian học KB trùng cửa sổ OOS (kề nhau thì được)."
  ),
  ...
  "bridge_roster": (
    ...
    "Xóa model sẽ gỡ id khỏi roster; id còn sót = *id ma* (nút Dọn roster)."
  ),
  "mt5_bridge": (
...
def glossary_sections() -> list[tuple[str, list[tuple[str, str]]]]:
  """Thuật ngữ & viết tắt dùng trong app (desk-aware)."""
  try:
