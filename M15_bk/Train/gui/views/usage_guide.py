...
from gui.navigation import ALL_ITEMS, LABEL_TAB_OOS, LABEL_TAB_REWARD
...
def _desk_ctx() -> dict:
  ...
  return {
    ...
    "tf": tf_label(),
    "ea_live": ea_live,
    "bridge_live": bridge_dir_display(BRIDGE_DIR),
    "bridge_sim": bridge_dir_display(BRIDGE_SIM_DIR),
    "magic": DEFAULT_MAGIC,
...
| **Trade Models** | Active (phân tích) · **Tổng hợp** (so sánh / Archive / đổi tên) · {LABEL_TAB_OOS} · Rủi ro · Nhật ký · Chiến lược |
| **Compare Trade** | So nhiều model trên lịch sử (**không EA**; sim fills) |
| **Live Trade** | Tab **Now** + roster · Risk control · Thống kê · chart · Health · test lịch sử · remine · Loss guard |
| **Hướng dẫn** | Trang này |
...
| ⑤ | Live Trade | Tab **Trade Models** → Start **Live** → theo dõi **Now** / Health |
...
"""
  ...
  st.markdown(
    f"""
**Active** (dropdown trên cùng) = model đang xem cho tab Thông tin / {LABEL_TAB_OOS} / Rủi ro / Nhật ký / Chiến lược.

**Tổng hợp**

- Bảng so sánh KPI · badge Live-ok / High-DD / Grid-only / Stale
...
"""
  ...
  # ── 5. Runtime pages ───────────────────────────────────────────
  st.markdown("## 5. Compare · Live Trade")
  st.markdown(
    f"""
### Compare Trade
So 1–N model trên cache {d['tf']} (`{d['cache']}`). Không EA. Kết quả = sim fills — **không** mở lệnh tài khoản.

### Live Trade (Now + Bridge)

1. Tab **Now**: pulse KPI, Start/Stop, desync, loss guard, chờ tín hiệu.
2. Tab **Trade Models**: chọn 1–{d['max_models']} model · readiness roster.
3. Tab **Risk control**: Risk % chung · Loss guard (ngưỡng ngày/tuần).
4. MT5: attach `{d['ea_live']}` · Live · Files/`{d['bridge_live']}`.
5. Mỗi {d['tf']}: EA `bar.json` → App decide → `decision.json` → BUY/SELL/FLAT → `fill.json` → `trades.json`.
6. Remine Live cùng đường Health OOS (KB pin, `conditions_fp`). Xem **Parity tuần này**.
7. Tab **Test lịch sử**: chỉ nhập from/to. App ghi `sim_control.json` vào `{d['bridge_live']}`. EA CopyRates, **cùng** `decide_for_bar` + loss guard như Live, **fill giấy** tại open nến (không OrderSend). Chart / lệnh / thống kê xem **Thống kê** / **Biểu đồ** (cùng folder). Tick Live tạm dừng đến khi Stop.
8. Một hàng tab: **Trade Models** · **Risk control** · **Thống kê** · **Biểu đồ** · **{LABEL_TAB_REWARD}** · **Kỹ thuật** · **Test lịch sử**.
...
| Khi dùng | Now + Thống kê + {LABEL_TAB_REWARD} | Test pipeline Live trên nến đã đóng |
"""
  )
