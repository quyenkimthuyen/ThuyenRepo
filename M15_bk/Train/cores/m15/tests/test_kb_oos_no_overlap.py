...
def test_adjacent_learn_oos_is_not_overlap():
  ...
  assert not kb_learn_overlaps_oos("2025-07-01", "2025-12-31", "2026-01-01", "2026-12-31")


def test_sanitize_defaults_both_oos_windows():
  cleaned = _sanitize_settings({"learning_eras": DEFAULT_LEARNING_ERAS})
  assert cleaned["oos_window_keys"] == ["2026", "2025-h2"]
  windows = resolve_oos_windows(cleaned)
  spans = {(w["oos_from"], w["oos_to"]) for w in windows}
  assert ("2026-01-01", "2026-12-31") in spans
  assert ("2025-07-01", "2025-12-31") in spans


def test_2025_h1_scores_both_windows_h2_skips_self():
  h1 = _settings(learning_era_keys=["2025-h1"])
  pairs = describe_kb_oos_pairs(h1)
  assert {(p["oos_key"], p["ok"]) for p in pairs} == {("2026", True), ("2025-h2", True)}
...
def test_build_grid_skips_overlapping_kb_oos(monkeypatch):
  ...
  windows = [("2026-01-01", "2026-12-31"), ("2025-07-01", "2025-12-31")]
  specs = build_grid(
    train_weeks=[8],
  ...
  assert ("era_2025_h2", "2026-01-01", "2026-12-31") in got
  assert ("era_2025_h2", "2025-07-01", "2025-12-31") not in got
  assert ("era_2025_h1", "2026-01-01", "2026-12-31") in got
...
def test_expected_count_uses_valid_pairs_only():
  ...
  # 2025-h2 × 2025-h2 OOS skipped → only 2026
  assert expected_grid_count_from_settings(h2) == 6


def test_add_custom_oos_window_to_catalog():
  s = _settings(learning_era_keys=["2025-h1"], oos_window_keys=["2026"])
  ...
  assert "2026" in s2["oos_window_keys"]
  ...
  assert ("2026-01-01", "2026-12-31") in spans3
