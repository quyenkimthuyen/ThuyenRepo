> **M5 desk `G33`** — `M5F4` · app/bridge/sim/compare `8831/9095/9206/9316` · folder `bridge_m5f4` · magic `20261561` · cloned from M15 `EdgeMinerGBPUSDM15`.

> **Backtest desk `G23`** — instance `M5F4` · app `8731` · Bridge `8995` · Sim `9106` · Compare `9216`
> (port `8996` = legacy Paper Monitor chart — desk đã gỡ) · folder `bridge_m5f4` · magic `20261023`.

# ForexForge — GBP/USD M15 Self-Learning Trading System

Hệ thống backtest walk-forward + self-learning cho **GBP/USD khung M15**, dùng thống nhất dữ liệu broker từ ForgeBridge/XM MT5.

> `ForgeBest3m_Frozen.mq5` và `ForgeBest3m_WF.mq5` là EA H1 legacy, không tương thích
> với model M15 và không được dùng cho live. Live/Simulate chỉ dùng `ForgeBridgeM5F4.mq5`.
...
M5F4: app `8731` · Bridge `8995` · Sim chart `9106` · Compare `9216`
(port `8996` legacy paper-chart — không dùng). Folder `bridge_m5f4`, Magic `20261023`.
...
App mặc định mở tại `http://127.0.0.1:8731`. Có thể đổi cổng bằng `-Port 8502` (Windows) hoặc `--port 8502` (Linux).
...
**“Paper” trong code** (`PaperBook`, `paper_fill.py`, port `8996`) = helper khớp lệnh mô phỏng cho Compare/HistoryFeed — **không** phải desk Paper Monitor (đã gỡ). Chi tiết: GUI **Hướng dẫn** mục 6.
...
data/mt5_gbpusd_m5.parquet  # Cache M15 chuẩn từ ForgeBridge/XM MT5
