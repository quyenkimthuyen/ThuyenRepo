#!/usr/bin/env python3
"""Unify all Trade Model comparisons on canonical OOS: 2026-01-01 → 2026-08-07.
...
- Writes ranked comparison under results/research/m5_oos_unified/

Usage:
  EdgeMinerM15B5/.venv/bin/python scripts/unify_oos_compare.py
  ... --reuse          # reuse existing per-model OOS report if window matches
  ... --archive-noncanonical   # archive models that had other oos windows before re-score
"""
...
OUT = ROOT / "results" / "research" / "m5_oos_unified"
ROUND3 = ROOT / "results" / "research" / "m5_round3_ensemble"
...
def patch_settings() -> None:
  ...
  ws["oos_to"] = OOS_TO
  # Prefer named Best* active when present
  from gui.trade_model import list_trade_models, set_active_trade_model
  by_label = {(m.get("label") or "").strip(): m for m in list_trade_models()}
  preferred = None
  for lab in ("BestQuality", "BestBalance", "BestPF", "BestTotalR"):
    if lab in by_label:
      preferred = by_label[lab]
      break
  if preferred:
  ...
  try:
    ...
    labels = [m.get("label") for m in list_trade_models() if m.get("label")]
    # Prefer Best* labels for compare UI
    best = [x for x in labels if str(x).startswith("Best")]
...
def load_cached_report(model_id: str) -> dict | None:
  candidates = [
    ...
    ROUND3 / f"{model_id}_oos_report.json",
...
def main() -> int:
  ...
  ap.add_argument("--archive-noncanonical", action="store_true",
                  help="Archive live models whose stored oos window was not canonical before update")
  args = ap.parse_args()
  ...
  if args.archive_noncanonical:
    for m in list(models):
      of = str(m.get("oos_from") or "")[:10]
      ot = str(m.get("oos_to") or "")[:10]
      if of != OOS_FROM or ot != OOS_TO:
        log(f"Archive non-canonical before rescore: {m.get('label')} ({of}→{ot})")
        archive_trade_model(m.get("id"))
    models = list_trade_models(include_archived=False)
