...
Trong app: sidebar **MT5 Bridge** — switch **Live | Simulate** (cùng desk/chart/thống kê/sức khỏe; khác `bridge_m15f1/` vs `bridge_sim_m15f1/`)
...
- **Simulate EA (History Feed)** — ForgeBridge **v1.05** `InpMode = HISTORY_FEED`, `InpBridgeSubdir = bridge_sim_m15f1`; App ghi `sim_control.json` (from/to/delay); EA `CopyRates` gửi bar/fill như live
...
Sau này khi sửa `mt5/Experts/ForgeBridgeM15F1.mq5`, chỉ chạy lệnh đầu tiên. Script
...
- Cần tài khoản **hedging**. Compile lại `ForgeBridgeM15F1` / `ForgeBridgeM15F1Sim` sau khi kéo code mới.
...
2. Inputs: `InpMode = HISTORY_FEED`, `InpBridgeSubdir = bridge_sim_m15f1` (junction/Files giống live).
3. App **MT5 Bridge → Simulate EA**: chọn from/to + delay → Start feed.
4. Service quyết định chạy trên `mt5/bridge_sim_m15f1/` (không đụng `bridge_m15f1/` live).
