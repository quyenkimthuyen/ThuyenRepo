...
Derived (example B4):
  ...
  EA              ForgeBridgeM5F3B4 / ForgeBridgeM5F3B4Sim
...
# Rename EA sources
if [[ -f "$DST_ROOT/mt5/Experts/ForgeBridgeM5F3.mq5" ]]; then
  mv "$DST_ROOT/mt5/Experts/ForgeBridgeM5F3.mq5" "$DST_ROOT/mt5/Experts/${EA_LIVE}.mq5"
fi
if [[ -f "$DST_ROOT/mt5/Experts/ForgeBridgeM5F3Sim.mq5" ]]; then
  mv "$DST_ROOT/mt5/Experts/ForgeBridgeM5F3Sim.mq5" "$DST_ROOT/mt5/Experts/${EA_SIM}.mq5"
fi
...
replace_file(
  ...
  [
    (r'^BRIDGE_DIR = ROOT / "mt5" / "bridge_m5f3"', f'BRIDGE_DIR = ROOT / "mt5" / "{bridge_live}"'),
    (r'^BRIDGE_SIM_DIR = ROOT / "mt5" / "bridge_sim_m5f3"', f'BRIDGE_SIM_DIR = ROOT / "mt5" / "{bridge_sim}"'),
    (r"^DEFAULT_MAGIC = \d+", f"DEFAULT_MAGIC = {magic_live}"),
    (r"^DEFAULT_SIM_MAGIC = \d+", f"DEFAULT_SIM_MAGIC = {magic_sim}"),
    (r'^INSTANCE_ID = "M5F3"', f'INSTANCE_ID = "{instance_id}"'),
  ],
)
...
# deploy script — placeholder rename to avoid ForgeBridgeM5F3 prefix clobbering Sim
deploy = root / "scripts" / "deploy_xm_forgebridge.ps1"
if deploy.exists():
  text = deploy.read_text(encoding="utf-8")
  text = text.replace("ForgeBridgeM5F3Sim", "@@EA_SIM@@")
  text = text.replace("ForgeBridgeM5F3", "@@EA_LIVE@@")
  ...
  text = text.replace('$BridgeSubdirLive = "bridge_m5f3"', f'$BridgeSubdirLive = "{bridge_live}"')
  text = text.replace('$BridgeSubdirSim = "bridge_sim_m5f3"', f'$BridgeSubdirSim = "{bridge_sim}"')
  text = re.sub(r'\$EaNameLive = "[^"]+"', f'$EaNameLive = "{ea_live}"', text)
  text = re.sub(r'\$EaNameSim = "[^"]+"', f'$EaNameSim = "{ea_sim}"', text)
  text = text.replace('$EaFolder = "EdgeMinerM5F3"', f'$EaFolder = "{repo_name}"')
...
# EA sources — use placeholders so ForgeBridgeM5F3 is not a prefix of the new Sim name.
for ea_path, subdir, magic, label in (
  ...
  text = text.replace("@@EA_LIVE@@", ea_live)
  text = re.sub(
    r'const string INSTANCE_ID = "M5F3";',
...
text = text.replace('assert \'const string INSTANCE_ID = "M5F3"\' in m15', f'assert \'const string INSTANCE_ID = "{instance}"\' in m15')
text = text.replace('assert \'const string INSTANCE_ID = "M5F3"\' in m15_sim', f'assert \'const string INSTANCE_ID = "{instance}"\' in m15_sim')
...
text = text.replace('(M15_ROOT, "ForgeBridgeM5F3", "ForgeBridgeM5F3Sim")', f'(M15_ROOT, "{ea}", "{ea_sim}")')
text = re.sub(
  r'm15 = \(M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM5F3\.mq5"\)',
  f'm15 = (M15_ROOT / "mt5" / "Experts" / "{ea}.mq5")',
  text,
)
text = re.sub(
  r'm15_sim = \(M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM5F3Sim\.mq5"\)',
