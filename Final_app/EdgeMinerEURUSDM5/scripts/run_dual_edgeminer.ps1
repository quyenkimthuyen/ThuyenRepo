...
$H1AppPort = 8502
$Expected = @{
  M15 = @{ Period = "M15"; Magic = 20261021; Instance = "M5F3"; Bridge = "bridge_m5f3" }
...
function Start-Services {
  Invoke-Python $M15Root "from mt5_bridge.background import save_config,start_worker; save_config(enabled=True,model_id='tm_m15_best_2_49216b56',risk_pct=1.0,poll_sec=2.0,bridge_dir=r'$M15Root\mt5\bridge_m5f3'); assert start_worker()"
...
function Enable-ExpertTrading {
  ...
  $names = @("ForgeBridgeM5F3", "ForgeBridgeM5F3Sim", "ForgeBridgeH1", "ForgeBridgeH1Sim")
...
function Deploy-Dual {
  ...
  Read-Connection (Join-Path $M15Root "mt5\bridge_m5f3\connection.json") $Expected.M15 | Out-Null
...
function Show-Status {
  ...
  foreach ($item in @(
    ...
    @{ Name = "M15 bridge"; Pattern = [regex]::Escape("$M15Root\mt5\bridge_m5f3"); Port = 8975 },
