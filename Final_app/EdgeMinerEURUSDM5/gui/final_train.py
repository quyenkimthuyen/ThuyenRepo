...
def filter_final_train_combos(
  ...
  *,
  ...
  """Drop promoted (optional) and combos with Max DD above ``max_dd_r``."""
...
def rank_final_train_combos(
  ...
  *,
  ...
  filtered = filter_final_train_combos(
    merged, hide_promoted=hide_promoted, max_dd_r=max_dd_r,
  )
  if not filtered:
    return []
  ...
  for i, r in enumerate(filtered):
    ...
    row["Preset"] = preset
    row["OOS"] = f"{row.get('oos_from') or '—'}→{row.get('oos_to') or '—'}"
