...
def portable_train_boot_cmd(desk: str) -> str:
...
def heal_autostart_launchers(app_root: Path, *, dry_run: bool = False) -> int:
  """Rewrite Train live_windows_boot.cmd files to relative paths."""
  runtime = app_root / "Train" / "runtime"
  ...
  for child in sorted(runtime.iterdir()):
    ...
    marker = results / "live_windows_autostart.json"
    if not cmd.is_file() and not marker.is_file():
      continue
    text = portable_train_boot_cmd(child.name)
    prev = ""
    if cmd.is_file():
      try:
        prev = cmd.read_text(encoding="utf-8")
      except (OSError, UnicodeError):
        prev = ""
    if prev.replace("\r\n", "\n") == text.replace("\r\n", "\n"):
      continue
    ...
    if not dry_run:
      ...
      cmd.write_text(text, encoding="ascii", newline="\r\n")
  return touched


def ensure_windows_autostart(app_root: Path) -> None:
  """Best-effort: rebind Windows Scheduled Tasks to this copy."""
  train = app_root / "Train"
  if train.is_dir():
    train_s = str(train)
    if train_s not in sys.path:
      sys.path.insert(0, train_s)
    try:
      from gui.live_autostart import ensure_live_autostart_after_move
      ensure_live_autostart_after_move()
    except Exception:
      pass
  live = app_root / "Trade" / "live"
  if live.is_dir():
    for p in (str(live), str(live.parent)):
      if p not in sys.path:
        sys.path.insert(0, p)
    try:
      from windows_autostart import ensure_autostart_after_move
      ensure_autostart_after_move()
    except Exception:
      pass
...
def heal_app_root(app_root: Path | None = None, *, dry_run: bool = False, force: bool = False) -> dict[str, Any]:
  ...
  if not moved and not force:
    ...
    summary["autostart_cmds"] = heal_autostart_launchers(root, dry_run=dry_run)
    if not dry_run:
      ensure_windows_autostart(root)
    return summary
  ...
  if not dry_run:
    write_marker(root)
    n_cmd = heal_autostart_launchers(root, dry_run=False)
    summary["autostart_cmds"] = n_cmd
    ensure_windows_autostart(root)
  else:
    summary["autostart_cmds"] = heal_autostart_launchers(root, dry_run=True)
  return summary
