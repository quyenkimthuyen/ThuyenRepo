...
def test_chart_recover_does_not_abort_later_live_tabs():
  """st.rerun inside chart recover used to leave Reward / Kỹ thuật / Test lịch sử empty."""
  root = Path(__file__).resolve().parents[3]
  bridge = (root / "gui" / "views" / "mt5_bridge.py").read_text(encoding="utf-8")
  recover = bridge[
    bridge.find("def _live_chart_recover_fragment"):
    bridge.find("def _render_live_chart")
  ]
  render = bridge[
    bridge.find("def _render_live_chart"):
    bridge.find("def _render_manual_test_orders")
  ]
  assert "_live_chart_recover_armed" in recover
  ...
  assert live.find('"Test lịch sử"') < live.find("render_tab_health()")
