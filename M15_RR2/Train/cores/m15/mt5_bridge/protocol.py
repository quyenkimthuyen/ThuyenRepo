...
EA_SYNC_NAME = "ea_sync.json"
DEALS_NAME = "deals.json"
...
INSTANCE_ID = str(_CFG.get("instance_id") or "RR2E21")
MAX_BRIDGE_MODELS = 5
...
def _normalize_bridge_path_text(raw: str | Path) -> str:
...
def bridge_dir_display(path: Path | str | None = None) -> str:
  """Runtime-relative bridge folder for UI, e.g. ``mt5/bridge_m15e21``.

  Avoids showing stale Windows absolutes on Linux (where ``Path.name`` can be
  the whole ``C:/...`` string).
  """
  if path is None:
    resolved = resolve_live_bridge_dir()
  else:
    text = _normalize_bridge_path_text(path)
    relocated = relocate_under_root(text, root=ROOT)
    resolved = Path(relocated) if relocated is not None else Path(text)
  try:
    abs_p = resolved.resolve()
  except (OSError, RuntimeError):
    abs_p = Path(_normalize_bridge_path_text(resolved))
  root = ROOT.resolve()
  try:
    return abs_p.relative_to(root).as_posix()
  except ValueError:
    parts = [p for p in abs_p.as_posix().split("/") if p]
    for i, part in enumerate(parts):
      if part.lower() == "mt5":
        return "/".join(parts[i:])
    name = parts[-1] if parts else "bridge"
    if name.lower().startswith("bridge"):
      return f"mt5/{name}"
    return name


def bridge_file_display(path: Path | str | None, filename: str) -> str:
  return f"{bridge_dir_display(path)}/{filename}"
...
def positions_path(bridge_dir: Path | None = None) -> Path:
  return ensure_bridge_dir(bridge_dir) / POSITIONS_NAME


def ea_sync_path(bridge_dir: Path | None = None) -> Path:
  return ensure_bridge_dir(bridge_dir) / EA_SYNC_NAME


def deals_path(bridge_dir: Path | None = None) -> Path:
  return ensure_bridge_dir(bridge_dir) / DEALS_NAME
