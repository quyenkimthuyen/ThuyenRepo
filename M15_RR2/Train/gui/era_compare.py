...
def _profile_epoch_count(profile_id: str) -> int:
  from kb_profiles import get_profile, load_kb, profile_path

  meta = get_profile(profile_id) or {}
  have = int(meta.get("epochs") or 0)
  path = profile_path(profile_id)
  if not path.exists():
    return have
  try:
    n = len(load_kb(profile_id).epoch_history)
  except Exception:
    return have
  return max(have, n)
...
def ensure_profile_learned(spec: dict, epochs: int, reset: bool = False) -> dict:
  ...
  Nếu profile đã có N epoch và yêu cầu M>N, chỉ chạy thêm M−N (không học lại từ đầu).
  """
  ...
  if profile_exists(p) and not reset:
    have = _profile_epoch_count(p)
    if have >= epochs:
      return {"skipped": True, "profile": p, "epochs": have}
    extra = int(epochs) - have
