...
SPLIT = LIVE.parent
sys.path.insert(0, str(LIVE / "gui"))
...
def _on_live_stats_period_change() -> None:
  from theme import save_ui_prefs
...
def _seed_live_stats_period() -> None:
  ...
  """
  from theme import load_ui_prefs, restore_widget_choice
...
def _render_now_side_rules(
  ...
  *,
  ...
  from theme import now_chart_check_id
...
def render_setup_page() -> None:
  ...
  with tab_quality:
    ...
    try:
      from theme import restore_widget_choice
  ...
  with tab_system:
    ...
    with st.expander("Autostart sau reboot", expanded=False):
      from windows_autostart import (
        ...
        ensure_autostart_after_move as _as_ensure,
      )
      if "_as_ensured" not in st.session_state:
        try:
          _as_ensure()
        except Exception:
          pass
        st.session_state["_as_ensured"] = True
      st_as = _as_status()
      ...
      if not _is_win_as():
        ...
        task_nm = st_as.get("task_name") or ""
        st.write(
          f"Task: **{task_nm or '—'}** · **{'INSTALLED' if installed else 'not installed'}** · "
...
with st.sidebar:
  ...
  from theme import load_ui_prefs, save_ui_prefs
