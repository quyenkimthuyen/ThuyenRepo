...
/** Kiểm tra message có chứa keyword nào */
function explorationMsgIncludes(message, keywords) {
  const m = (message || '').toLowerCase();
  return keywords.some((kw) => m.includes(kw.toLowerCase()));
}

/**
 * Rule khớp insight → gợi ý suy ngẫm tiếp (id trùng I18N_EXPLORATION_PROMPTS)
 * priority thấp hơn = ưu tiên cao hơn
 */
const EXPLORATION_PROMPT_RULES = [
  {
    id: 'family_work',
    priority: 1,
    matches(insights) {
      return insights.contradictions.some(
        (c) =>
          explorationMsgIncludes(c.message, ['gia đình', 'family', 'ưu tiên']) &&
          explorationMsgIncludes(c.message, ['làm việc', '14 giờ', 'overtime', 'thời gian', 'công việc', 'work'])
      );
    },
  },
  {
    ...
    matches(insights) {
      return insights.contradictions.some(
        (c) =>
          explorationMsgIncludes(c.message, ['sức khỏe', 'health', 'khỏe mạnh']) &&
          explorationMsgIncludes(c.message, ['stress', 'không ngủ', 'mệt', 'căng thẳng', 'ảnh hưởng'])
      );
    },
  },
  {
    ...
    priority: 3,
    matches(insights) {
      return insights.contradictions.some((c) =>
        explorationMsgIncludes(c.message, ['cân bằng', 'balance', 'work-life', 'nghỉ'])
      );
    },
  },
  {
    id: 'development_pressure',
    priority: 4,
    matches(insights) {
      return insights.contradictions.some((c) =>
        explorationMsgIncludes(c.message, ['phát triển', 'học hỏi', 'development', 'áp lực', 'ép học', 'bắt con'])
      );
    },
  },
  {
    ...
    matches(insights) {
      return insights.contradictions.some((c) =>
        explorationMsgIncludes(c.message, ['tự do', 'tự chủ', 'freedom', 'kiểm soát', 'control'])
      );
    },
  },
  {
    ...
    matches(insights) {
      return insights.contradictions.some((c) =>
        explorationMsgIncludes(c.message, ['nghe lời', 'obey', 'obedien'])
      );
    },
  },
  {
    ...
    matches(insights) {
      return insights.contradictions.some((c) =>
        explorationMsgIncludes(c.message, ['tiền', 'money', 'hạnh phúc', 'happiness'])
      );
    },
  },
  {
    ...
    priority: 10,
    matches(insights) {
      return insights.biases.some((b) => b.labelEn === 'Catastrophizing');
    },
  },
  {
    id: 'bias_should',
    priority: 11,
    matches(insights) {
      return insights.biases.some((b) => b.labelEn === 'Should Statements');
    },
  },
  {
    id: 'bias_overgeneralization',
    priority: 12,
    matches(insights) {
      return insights.biases.some((b) => b.labelEn === 'Overgeneralization');
    },
  },
  {
    id: 'bias_mind_reading',
    priority: 13,
    matches(insights) {
      return insights.biases.some((b) => b.labelEn === 'Mind Reading');
    },
  },
  {
    id: 'general_belief',
    priority: 90,
    matches(insights) {
      return insights.topBeliefs.length > 0;
    },
    vars(insights) {
      return { belief: insights.topBeliefs[0].label };
    },
  },
];
