...
def pips_to_points(spread_pips: float) -> int:
  """Convert pips → 5-digit points (1.0 pip = 10 points)."""
  try:
    pips = max(0.0, float(spread_pips or 0.0))
  except (TypeError, ValueError):
    return 0
  return int(round(pips * (PIP / POINT)))
