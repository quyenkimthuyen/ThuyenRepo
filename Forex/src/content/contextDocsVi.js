    ...
    blocks: [
      ...
      { type: 'h3', text: 'Nguồn liquidity (kiểm tra đồng thời)' },
      {
        ...
        rows: [
          ...
          ['Swing', 'Luôn (fallback)', 'Swing high/low lookback ngắn'],
        ],
      },
      ...
      },
      {
        ...
        rows: [
          ...
          ['Volatility', 'Bắt buộc volOk', 'Không lọc'],
          ['RR mặc định', '1.5', '2'],
        ],
      },
      {
        ...
        rows: [
          ...
          ['wickRatio', '0.65', 'Râu rejection trên nến sweep'],
          ['confirmMaxBars', '2', 'Chờ tối đa bao nhiêu nến sau sweep'],
          ['minVolatilityRatio', '0.95', 'Chỉ trade khi biến động đủ lớn'],
          ['usePrevAsian', 'true', 'Dùng biên Á hôm trước lúc London mở'],
          ['swingLookback', '10', 'Swing fallback khi chưa có session range'],
      ...
      },
      {
        type: 'callout',
        variant: 'tip',
        text: 'Gợi ý workflow: EURUSD H1 → Suggest from data (Optimizer) → Grid grabPips / wickRatio / minVol → Sensitivity → Walk Forward. Backtest mặc định ~64 lệnh/3 năm, WR ~52% (in-sample) — không phải lời khuyên đầu tư.',
      },
      {
        type: 'callout',
        variant: 'warn',
    ...
    blocks: [
      ...
      },
      {
        ...
        rows: [
          ...
          ['Session Liquidity Sweep', 'Min Volatility Ratio', 'Lọc thị trường biến động thấp'],
          ['Session Liquidity Sweep', 'Use Prev Asian', 'Dùng range phiên Á ngày trước'],
        ],
      },
    ...
    blocks: [
      ...
      },
      {
        ...
        rows: [
          ...
          ['Grid Search', 'Thử nhiều combo cài đặt, xếp hạng kết quả — từ ≥4 combo chạy song song (parallel)'],
          ['Session Liquidity Sweep', 'Fade quét liquidity biên phiên (Asian/London) — entry 2 pha sau nến xác nhận'],
          ['Sensitivity', 'Biểu đồ WR / Expectancy / Net Profit theo từng giá trị param sau Grid Search'],
          ['Apply best to Strategy', 'Ghi combo #1 từ Grid Search vào tham số strategy đang chọn'],
      ...
      },
      {
        type: 'callout',
        variant: 'tip',
        text: 'Mở Optimizer (Ctrl+8) — phần đầu cũng có bảng từ ngữ tương tự, gắn trực tiếp với 4 tab Grid Search, Sensitivity, Walk Forward, Monte Carlo.',
      },
    ],
  },
  {
