...
from data_loader import META_PATH, load_gbpusd_h1, download_gbpusd_h1
...
def refresh_market_data(start: str = DEFAULT_START_DATE) -> pd.DataFrame:
  ...
  return download_gbpusd_h1(start, force_refresh=True)
...
def execute_backtest(
  ...
  rp = reopt_period or get_settings().get("reopt_period", "week")

  df = load_gbpusd_h1(start_date)
  reset_kb_cache()
  if use_learning:
...
def execute_learning(
  ...
  try:
    from gui.kb_learning_store import archive_learning_report
    report["kb_name"] = kb_name or kb_profile
    archive_learning_report(report)
  except Exception:
    pass
  try:
