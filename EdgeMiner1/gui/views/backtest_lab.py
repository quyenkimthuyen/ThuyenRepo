...
from gui.components import era_workflow_panel
...
def render():
  ...
  st.caption("Walk-forward · chọn KB profile + giai đoạn OOS (xem **KB & Giai đoạn** để quản lý profiles)")
  ...
  with st.sidebar:
    ...
    start_date = st.text_input("Data từ", "2022-01-01", key="lab_start")
    ...
    save_to_compare = st.checkbox("Lưu vào Report Compare", value=False, key="lab_archive")

    era = era_workflow_panel("lab", in_sidebar=True)
    use_kb = era["use_kb"]
    kb_profile = era["kb_profile"]
    kb_snapshot = era.get("kb_snapshot")
    oos_from = era["oos_from"]
    oos_to = era["oos_to"]
  ...
  if kv.get("message"):
    st.caption(f"KB check: {kv['message']}")
