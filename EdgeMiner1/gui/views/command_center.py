...
from gui.components import (
  ...
  list_kb_profiles_df, oos_period_inputs,
)
...
def render():
  ...
  with st.expander("KB Profiles (giai đoạn)", expanded=False):
    pdf = list_kb_profiles_df()
    if pdf.empty:
      st.caption("Chưa có profile — tạo ở **KB & Giai đoạn**.")
    else:
      show = [c for c in ["id", "name", "trained_from", "trained_to", "epochs"] if c in pdf.columns]
  ...
  learn_profile = kb_profile_picker("cc_learn", show_meta=False)
  epochs = st.number_input("Learning epochs", 1, 20, 2, key="cc_epochs")

  if use_kb:
    kb_profile, kb_snapshot = kb_profile_and_epoch_picker("cc_bt", show_meta=True)
    st.markdown("**Giai đoạn OOS**")
    oos_from, oos_to = oos_period_inputs("cc", kb_profile)
    if kb_profile:
      kb_validation_banner(kb_profile, oos_from or "2022-04-01")
    ...
    if st.button("▶ Run Backtest", type="primary", use_container_width=True):
      ...
      with st.spinner("Đang chạy walk-forward..."):
        report = execute_backtest(
          ...
          kb_profile=kb_profile or "default",
  ...
  with b2:
    if st.button("🧠 Run Learning", use_container_width=True):
      ...
      p = get_profile(learn_profile or DEFAULT_PROFILE_ID)
      with st.spinner(f"Chạy {epochs} epoch trên {learn_profile}..."):
        def on_ep(ep, total, state):
          prog.progress(ep / total, text=f"Epoch {ep}/{total}")

        execute_learning(
          epochs=int(epochs),
          kb_profile=learn_profile or DEFAULT_PROFILE_ID,
      ...
      prog.empty()
      st.success(f"Hoàn thành {epochs} epoch — profile **{learn_profile}**")
      st.rerun()
