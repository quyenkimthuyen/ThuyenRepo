...
WORKFLOW_STEPS: tuple[dict, ...] = (
  ...
  },
  {
    ...
    "detail": "Bật bộ nhớ, thử nhiều giai đoạn/epoch — lưu mỗi lần chạy.",
    "nav_page": "research",
    "research_tab": "era",
  },
  {
    ...
    "detail": "So sánh báo cáo — chọn combo tốt (R, sụt giảm, PF).",
    "nav_page": "research",
    "research_tab": "reports",
  },
  {
    "id": 4,
    "key": "paper",
    ...
    "short": "Paper",
    "subtitle": "Đúng Trade Profile đã chọn",
...
def apply_report_to_profile(report_id: str) -> dict:
  """Áp config từ báo cáo đã chọn → Trade Profile active."""
  from gui.report_store import load_report
  from gui.trade_profile import get_active_trade_profile, set_active_trade_profile
  ...
  fields = {
    ...
    "backtest_total_r": o.get("total_r"),
    "source": "workflow_pick",
  }
  tp = set_active_trade_profile(**{k: v for k, v in fields.items() if v is not None})

  state = load_workflow_state()
  state["chosen_report_id"] = report_id
  state["chosen_profile_id"] = tp.get("id")
  save_workflow_state(state)
  return {"ok": True, "profile_id": tp.get("id"), "label": tp.get("label")}
...
def assess_workflow() -> dict:
  ...
  if state.get("chosen_profile_id") and tp.get("id") == state["chosen_profile_id"]:
    step3_done = True
