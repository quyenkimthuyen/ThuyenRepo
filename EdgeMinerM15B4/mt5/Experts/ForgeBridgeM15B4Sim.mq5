//+------------------------------------------------------------------+
//| ForgeBridgeM15B4Sim.mq5 — EdgeMiner M15B4 SIM (magic 20262004, bridge_sim_m15b4) |
...
input string InpBridgeSubdir   = "bridge_sim_m15b4";    // under MQL5/Files/ (SIM HistoryFeed)
...
int OnInit()
{
   ...
   else if(InpMode == BRIDGE_HISTORY_FEED)
   {
      ...
      {
         Print("ForgeBridgeM15B4 requires a hedging account.");
         return INIT_FAILED;
      }
      g_sim_ea_status = "idle";
      WriteSimControlFile();
      EventSetMillisecondTimer(50);
      Print("ForgeBridgeM15B4 HistoryFeed | Files/", InpBridgeSubdir,
   ...
   else
   {
      ...
      Print("ForgeBridgeM15B4 Live | Files/", InpBridgeSubdir, " | magic=", InpMagic);
   }
...
bool LoadHistoryRatesRange()
{
   ...
   g_open_signal_id = "";
   g_had_position = false;
   Print("ForgeBridgeM15B4 HistoryFeed loaded bars=", g_hist_n,
...
bool PaperOpenFromDecision(const string json, const double entry_price, const string bar_time = "")
{
   ...
   WriteFillJsonEx("open", sid, action, true, "opened",
                   ...
                   false, "strategy", bt);
   Print("ForgeBridgeM15B4 HistoryFeed paper ", action, " @", entry_price,
...
void ProcessHistoryFeed()
{
   ...
   if(g_hist_n < 1 || g_hist_cursor >= g_hist_n)
   {
      ...
      g_sim_enabled = false;
      WriteSimControlFile();
      Print("ForgeBridgeM15B4 HistoryFeed completed bars=", g_hist_n);
      return;
   }
   ...
   if(flat && g_pending_decision == "")
   {
      ...
      if(WaitDecisionForBar(want, json, wait_ms))
      {
         ...
         Print("ForgeBridgeM15B4 HistoryFeed: no decision for ", want,
