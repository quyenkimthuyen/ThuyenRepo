...
Derived (example B4):
  ...
  EA              ForgeBridgeM15B4 / ForgeBridgeM15B4Sim
...
# Rename EA sources
if [[ -f "$DST_ROOT/mt5/Experts/ForgeBridgeM15.mq5" ]]; then
  mv "$DST_ROOT/mt5/Experts/ForgeBridgeM15.mq5" "$DST_ROOT/mt5/Experts/${EA_LIVE}.mq5"
fi
if [[ -f "$DST_ROOT/mt5/Experts/ForgeBridgeM15Sim.mq5" ]]; then
  mv "$DST_ROOT/mt5/Experts/ForgeBridgeM15Sim.mq5" "$DST_ROOT/mt5/Experts/${EA_SIM}.mq5"
fi
...
# deploy script — placeholder rename to avoid ForgeBridgeM15 prefix clobbering Sim
deploy = root / "scripts" / "deploy_xm_forgebridge.ps1"
if deploy.exists():
  text = deploy.read_text(encoding="utf-8")
  text = text.replace("ForgeBridgeM15Sim", "@@EA_SIM@@")
  text = text.replace("ForgeBridgeM15", "@@EA_LIVE@@")
  ...
  text = text.replace('$BridgeSubdirLive = "bridge"', f'$BridgeSubdirLive = "{bridge_live}"')
  text = text.replace('$BridgeSubdirSim = "bridge_sim"', f'$BridgeSubdirSim = "{bridge_sim}"')
...
# EA sources — use placeholders so ForgeBridgeM15 is not a prefix of the new Sim name.
for ea_path, subdir, magic, label in (
  ...
  text = text.replace("@@EA_LIVE@@", ea_live)
  text = re.sub(
    r'const string INSTANCE_ID = "M15";',
...
text = text.replace('assert \'const string INSTANCE_ID = "M15"\' in m15', f'assert \'const string INSTANCE_ID = "{instance}"\' in m15')
text = text.replace('assert \'const string INSTANCE_ID = "M15"\' in m15_sim', f'assert \'const string INSTANCE_ID = "{instance}"\' in m15_sim')
...
text = text.replace('(M15_ROOT, "ForgeBridgeM15", "ForgeBridgeM15Sim")', f'(M15_ROOT, "{ea}", "{ea_sim}")')
text = re.sub(
  r'm15 = \(M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM15\.mq5"\)',
  f'm15 = (M15_ROOT / "mt5" / "Experts" / "{ea}.mq5")',
  text,
)
text = re.sub(
  r'm15_sim = \(M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM15Sim\.mq5"\)',
