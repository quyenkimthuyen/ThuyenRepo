...
/** Datasets loaded first so BTC chart works quickly on boot. */
const BOOT_PRIORITY = Object.freeze([
  { symbol: 'BTCUSD', timeframe: 'W' },
...
export async function loadDefaultDatasets(saveCandles, listDatasets, getCandleCount, options = {}) {
  ...
  if (typeof window !== 'undefined' && window.location.protocol === 'file:') {
    result.errors.push(
      'Opened via file:// — data cannot load. Run: cd LongBTC && python3 -m http.server 8080 then open http://localhost:8080'
    );
  ...
  if (!manifest?.datasets?.length) {
    ...
    result.errors.push(
      `Cannot reach data/defaults/manifest.json. Tried: ${bases}. Copy the full LongBTC folder including data/defaults/.`
    );
  }
