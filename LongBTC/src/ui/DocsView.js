...
import { LONG_BTC_DOC_SECTIONS, docSectionForView, getLongBtcDoc } from '../content/longbtcDocsVi.js';
...
/** @type {string} */
let activeSection = LONG_BTC_DOC_SECTIONS[0]?.id ?? 'overview';

/**
 * @param {string} sectionId
 */
export function setDocsSection(sectionId) {
  if (LONG_BTC_DOC_SECTIONS.some((s) => s.id === sectionId)) {
...
class DocsViewImpl {
   ...
   */
  #renderNav() {
    return LONG_BTC_DOC_SECTIONS.map((section) => {
      const btn = el('button', {
  ...
  #renderSection() {
    ...
    const section = LONG_BTC_DOC_SECTIONS.find((s) => s.id === activeSection);
