...
const ReflectionEngine = {
  ...
  /**
   * Áp dụng trích xuất từ AI vào cây nhận thức
   */
  applyAiExtracted(text, aiExtracted, extracted, nodesCreated, nodeIds) {
    const upsertMany = (type, labels, key) => {
      for (const label of (labels || []).slice(0, 3)) {
        ...
        extracted[key].push(node);
        nodesCreated.push(node);
        nodeIds.push(node.id);
      }
    };

    if (aiExtracted?.interpretation) {
      const label =
        aiExtracted.interpretation.length > 60
          ? `${aiExtracted.interpretation.slice(0, 60)}...`
          : aiExtracted.interpretation;
      const { node } = CognitiveTree.upsertNode({
        type: 'Interpretation',
        label,
        sourceText: text,
    ...
    upsertMany('Emotion', aiExtracted?.emotions, 'emotions');
    upsertMany('Belief', aiExtracted?.beliefs, 'beliefs');
    upsertMany('Value', aiExtracted?.values, 'values');
    upsertMany('Identity', aiExtracted?.identity, 'identity');
    upsertMany('Action', aiExtracted?.actions, 'actions');
  },

  /**
   * Xử lý tin nhắn bằng Cursor AI (câu hỏi) + rule engine (ghi nhận node)
   */
  async processMessageWithAi(text, session) {
    const ruleResult = this.processMessageRule(text, session);
    const ai = await AiClient.reflect({
      message: text,
      session,
      phase: 'continue',
    });
    ...
    const flow = CognitiveLibrary.REFLECTION_FLOW;
    const flowStep = flow.includes(ai.flowStep) ? ai.flowStep : ruleResult.flowStep;

    let nextQuestion = ai.nextQuestion || ruleResult.nextQuestion;
    if (ai.sessionEnd) {
      nextQuestion = this.buildSessionEndMessage(session);
    }

    if (ai.extracted) {
      this.applyAiExtracted(
        text,
        ai.extracted,
        ruleResult.extracted,
        ruleResult.nodesCreated,
        ruleResult.nodeIds
      );
    }
    ...
    return {
      ...ruleResult,
      flowStep,
      previousStep: session.flowStep || ruleResult.previousStep,
      nextQuestion,
      guidePrefix: this.getGuidePrefix(flowStep),
      aiPowered: true,
    };
  },

  /**
   * Xử lý tin nhắn user — rule engine
   * @returns {Object} Kết quả xử lý
   */
  processMessageRule(text, session) {
    const flow = CognitiveLibrary.REFLECTION_FLOW;
   ...
   * Tạo session mới
   */
  async createSession(initialThought, options = {}) {
    const session = {
    ...
    const useAi =
      !options.forceRule && typeof AiClient !== 'undefined' && AiClient.isEnabled();

    let result;
    let openingContent;
    let openingFlowStep = 'Emotion';

    if (useAi) {
      try {
        result = this.processMessageRule(initialThought, session);
        const ai = await AiClient.reflect({
          message: initialThought,
          session,
          phase: 'opening',
        });
        openingContent = ai.nextQuestion || this.getOpeningQuestion(initialThought);
        openingFlowStep = CognitiveLibrary.REFLECTION_FLOW.includes(ai.flowStep)
          ? ai.flowStep
          : 'Emotion';
        if (ai.extracted) {
          this.applyAiExtracted(
        ...
        result = this.processMessageRule(initialThought, session);
        openingContent = this.getOpeningQuestion(initialThought);
      }
    } else {
      result = this.processMessageRule(initialThought, session);
      openingContent = this.getOpeningQuestion(initialThought);
    ...
    session.flowStep = openingFlowStep;
   ...
   */
  async continueSession(sessionId, userText, options = {}) {
    ...
    if (useAi) {
      try {
        result = await this.processMessageWithAi(userText, session);
      } catch {
        result = this.processMessageRule(userText, session);
        aiFallback = true;
      }
    } else {
      result = this.processMessageRule(userText, session);
    ...
    return { session, result, guideMessage: guideMsg, aiFallback };
  },
