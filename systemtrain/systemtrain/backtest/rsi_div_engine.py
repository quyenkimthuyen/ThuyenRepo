"""Backtest RSI divergence — SL at swing extreme + ATR buffer, fixed RR."""
...
class RsiDivEngine:
    def __init__(
        ...
        div_config: RsiDivConfig | None = None,
        ...
        self.div_config = div_config or RsiDivConfig()
        self.initial_equity = initial_equity
        self.spread = spread_pips * pip_size
        self.slippage = slippage_pips * pip_size
        self.rr = self.div_config.rr
        self.pip_size = pip_size

    def run(self, df: pd.DataFrame) -> BacktestResult:
        cfg = self.div_config
        signals = detect_rsi_divergence_signals(df, cfg)
        if not signals:
        ...
        for i in range(1, n):
            ...
            if open_trade is None and i in entry_map and risk_mgr.can_open_trade(0):
                ...
                raw_entry = opens[i] if cfg.entry_delay_bars > 0 else closes[i]
