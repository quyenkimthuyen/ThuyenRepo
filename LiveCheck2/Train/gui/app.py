...
from gui.views import (
  ...
  learning_hub, trade_models_view, compare_trade, live_trade_dash,
)
from gui.navigation import (
  ALL_ITEMS, LEGACY_ALIASES, NAV_ITEMS,
  LEARNING_TAB_BY_ALIAS, ANALYSIS_TAB_BY_ALIAS, default_page_key,
)
from gui.ui_theme import icon_btn, inject_theme, render_sidebar_brand
...
def _resolve_page_key() -> str:
  ...
  if key == "analysis":
    ...
    return "models"
  if key == "paper" or key == "mt5_bridge":
...
def main():
  try:
    ...
    _title = f"ForexForge · {DEFAULT_PAIR} {DEFAULT_TF}"
  except Exception:
    _title = "ForexForge"
  st.set_page_config(
    page_title=_title,
  ...
  render_sidebar_brand(desk=desk_caption())
