...
def load_models_store() -> dict:
  ...
  store = {"models": models}
  if purged_ids or len(models) != len(raw):
...
def list_trade_models() -> list[dict]:
  """Registry models in trade_models.json."""
  return list(load_models_store().get("models") or [])
...
def known_live_model_ids() -> set[str]:
  """Ids eligible for Bridge / Active."""
  return {str(m["id"]) for m in list_trade_models() if m.get("id")}


def bridge_ghost_model_ids() -> list[str]:
  """Bridge config ids missing from live store (deleted)."""
...
def prune_bridge_roster(
  *,
  ...
  """Remove deleted/explicit ids from Bridge ``model_ids``.
...
def bridge_remine_status_label(
  source: str | None,
  *,
  remine_each_week: bool = True,
) -> str:
  """Human label for Bridge weekly strategy source (Trade Models roster)."""
  if source == "schedule":
    return "OOS schedule"
...
def bridge_roster_display_rows(
  *,
  ...
  remine_each_week = bool(load_config().get("remine_each_week", True))
  ref_bar = status.get("last_bar") or (read_json(status_path(alt_dir)) or {}).get("last_bar")
  ...
  for mid in ids:
    ...
    pm = per.get(mid) or {}
    week_start = pm.get("week_start")
    remine_source = pm.get("remine_source")
    strategy = (pm.get("strategy_name") or "")[:36] if include_runtime else None
    if include_runtime and not week_start and ref_bar:
      try:
        ws, _we = _week_bounds_for_ts(pd.Timestamp(ref_bar))
        week_start = str(ws.date())
      except Exception:
        week_start = None
    if include_runtime and not remine_source and week_start:
      entry, src = lookup_week_strategy_with_source(mid, week_start)
      if entry:
        remine_source = src
        if not strategy:
          strat_d = entry.get("strategy") if isinstance(entry.get("strategy"), dict) else {}
          strategy = str(strat_d.get("name") or "")[:36] or None
      elif remine_each_week:
        remine_source = "pending"
      else:
        remine_source = "pending"
    rows.append({
      "id": mid,
      ...
      "strategy": strategy,
      "week_start": week_start if include_runtime else None,
      "remine_source": remine_source if include_runtime else None,
      "remine_each_week": remine_each_week if include_runtime else None,
      "remine_status": (
        bridge_remine_status_label(remine_source, remine_each_week=remine_each_week)
...
def desk_pair_code() -> str:
  ...
  try:
    ...
    if os.environ.get("TRAINAPP_DESK"):
      ...
      score = str(load_desk().get("score_desk") or "").upper()
      if score in ("EUR", "GBP"):
        return score
  except Exception:
    pass
  try:
    ...
    pair = str(DEFAULT_PAIR or "").upper()
    if "GBP" in pair:
      return "GBP"
    if "EUR" in pair:
      return "EUR"
...
def overview_row_visible(
  ...
  *,
  ...
  """Default: only Active + Bridge. Catalog needs Hiện tất cả."""
  if show_all:
    return True
...
def model_from_grid_row(row: dict, *, run_id: str | None = None, label: str | None = None) -> dict:
  ...
  if (
    data_meta.get("source") not in ("mt5_ea", "mt5_api")
    or data_meta.get("timeframe") != DEFAULT_TF
...
def trade_model_to_workspace(m: dict | None = None) -> dict:
  ...
  return {
    ...
    "feature_profile": (
      ...
      or ("legacy" if int(m.get("feature_schema") or 0) < 3 else DEFAULT_FEATURE_PROFILE)
    ),
