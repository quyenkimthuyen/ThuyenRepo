...
def _worker_model_health(state: dict):
  ...
  spread = float(p.get("spread_pips") or model.get("spread_pips") or DEFAULT_SPREAD_PIPS)
  slip = float(p.get("slippage_pips") or model.get("slippage_pips") or DEFAULT_SLIPPAGE_PIPS)
