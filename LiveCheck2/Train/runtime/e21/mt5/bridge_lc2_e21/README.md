...
Trong app: sidebar **MT5 Bridge** — switch **Live | Simulate** (cùng desk/chart/thống kê/sức khỏe; khác `bridge_m15e21/` vs `bridge_sim_m15e21/`)
...
- **Test lịch sử** — cùng Live EA; App ghi `sim_control.json` vào `bridge_m15e21` (from/to/delay); EA `CopyRates` + fill giấy
...
# Test lịch sử: cùng Live EA — dùng tab Test lịch sử trong App (sim_control.json)
```

Sau này khi sửa EA, cập nhật template rồi render cho mọi desk:
...
EA desk hiện tại **v1.25** (parallel multi-model wait, late catch-up, `ea_sync.json`).
Chỉ sửa `mt5/Experts/ForgeBridge*.mq5` trực tiếp khi hotfix nhanh — lần sau chạy render lại.
...
- Cần tài khoản **hedging**. Compile lại `ForgeBridgeM15E21` sau khi kéo code mới.
- Live Trade dashboard: KPI tổng + bảng từng model.

## History Feed (Test lịch sử — cùng Live EA)

1. Gắn `ForgeBridgeM15E21` (Live) trên chart; junction/Files trỏ `bridge_m15e21`.
2. App **MT5 Bridge → Test lịch sử**: chọn from/to + delay → Start feed.
3. App ghi `sim_control.json` vào **cùng** folder `bridge_m15e21/`; EA CopyRates và fill giấy (không OrderSend).
