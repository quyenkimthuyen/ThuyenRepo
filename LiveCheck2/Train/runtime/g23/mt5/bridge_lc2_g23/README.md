...
- **Test lịch sử** — cùng Live EA; App ghi `sim_control.json` vào `bridge_m15g23` (from/to/delay); EA `CopyRates` + fill giấy
...
- Cần tài khoản **hedging**. Compile lại `ForgeBridgeM15G23` sau khi kéo code mới.
- Live Trade dashboard: KPI tổng + bảng từng model.

## History Feed (Test lịch sử — cùng Live EA)

1. Gắn `ForgeBridgeM15G23` (Live) trên chart; junction/Files trỏ `bridge_m15g23`.
2. App **MT5 Bridge → Test lịch sử**: chọn from/to + delay → Start feed.
3. App ghi `sim_control.json` vào **cùng** folder `bridge_m15g23/`; EA CopyRates và fill giấy (không OrderSend).
