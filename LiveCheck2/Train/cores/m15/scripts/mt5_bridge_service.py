...
def main() -> int:
  ...
  try:
    ...
    print(f"[bridge] startup position_sync skip: {sync_exc}", flush=True)
  try:
    from mt5_bridge.weekend_preremine import maybe_preremine_engines
    pre = maybe_preremine_engines(engines, bridge_dir=bridge_dir)
    if not pre.get("skipped"):
      print(
        f"[bridge] weekend_preremine week={pre.get('week_start')} "
        f"ok={pre.get('ok')} models={len(pre.get('models') or [])}",
        flush=True,
      )
  except Exception as pre_exc:
    print(f"[bridge] weekend_preremine skip: {pre_exc}", flush=True)
  ...
  while True:
    try:
      ...
      try:
        ...
        feed_active = False
      if not args.once and not feed_active:
        now_chk = time.time()
        if (now_chk - last_preremine_check) >= 60.0:
          last_preremine_check = now_chk
          try:
            from mt5_bridge.weekend_preremine import maybe_preremine_engines
            maybe_preremine_engines(engines, bridge_dir=bridge_dir)
          except Exception as pre_exc:
            print(f"[bridge] weekend_preremine skip: {pre_exc}", flush=True)
