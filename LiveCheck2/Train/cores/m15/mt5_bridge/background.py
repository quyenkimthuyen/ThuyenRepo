...
def _cfg_int_map(raw) -> dict[str, int]:
...
def load_config() -> dict:
  ...
  return {
    ...
    "max_trades_per_day_by_model": _cfg_int_map(data.get("max_trades_per_day_by_model")),
    # Live Bridge: remine unseen broker weeks (default ON). OFF = freeze_first after
    # first live remine in the worker process (still uses OOS schedule when present).
    "remine_each_week": bool(data.get("remine_each_week", True)),
    # Pre-remine next broker week on Fri evening / Sat / Sun (quiet market).
    "weekend_preremine_enabled": bool(data.get("weekend_preremine_enabled", True)),
  }


_CFG_CACHE: dict | None = None
_CFG_CACHE_AT: float = 0.0
_CFG_CACHE_TTL_SEC = 2.0


def invalidate_config_cache() -> None:
  """Drop cached bridge config (call after save_config / external edits)."""
  global _CFG_CACHE, _CFG_CACHE_AT
  _CFG_CACHE = None
  _CFG_CACHE_AT = 0.0


def load_config_cached(*, force: bool = False, max_age_sec: float | None = None) -> dict:
  """Cached ``load_config`` for hot paths (engine decisions, weekend preremine)."""
  global _CFG_CACHE, _CFG_CACHE_AT
  ttl = _CFG_CACHE_TTL_SEC if max_age_sec is None else float(max_age_sec)
  now = time.monotonic()
  if force or _CFG_CACHE is None or (now - _CFG_CACHE_AT) > ttl:
...
def _cycle(
  ...
  """One decide loop for Live and history replay.

  ``is_sim`` only reduces log/config I/O. Loss guard, halt, and
  ``decide_for_bar`` are the same path. Execution difference is EA-side
  (CopyRates + paper fill vs live ticks + OrderSend).
  """
  ...
  if not is_sim:
    ...
    if ingested_any_fill:
      ...
      sync = reconcile_journal_from_ea_positions(
        bridge_dir,
        connection=conn,
        require_fresh_heartbeat=not user_redecide,
      )
      n_ghost = int(sync.get("closed_ghosts") or 0)
      if (
        sync.get("updated")
        or sync.get("imported")
        or sync.get("repaired")
        or int(sync.get("closed") or 0) > 0
      ):
        need_redecide = True
        for eng in eng_map.values():
          eng._last_bar_key = None
  ...
  for mid, eng in eng_map.items():
    ...
    per_model[mid] = {
      ...
      "remine_source": decision.get("remine_source"),
      "remine_each_week": decision.get("remine_each_week"),
    }
...
def _worker():
  ...
  try:
    ...
    try:
      from mt5_bridge.position_sync import startup_live_reconcile
      startup_live_reconcile(bridge_dir)
    except Exception as sync_exc:
      append_event(
        "system", "position_sync_skip", bridge_dir=bridge_dir, summary=str(sync_exc),
      )
    try:
      from mt5_bridge.weekend_preremine import maybe_preremine_engines
      pre = maybe_preremine_engines(_engines, bridge_dir=bridge_dir)
      if not pre.get("skipped"):
        append_event(
          "system", "weekend_preremine_start", bridge_dir=bridge_dir,
          summary=f"week={pre.get('week_start')} ok={pre.get('ok')}",
          payload=pre,
        )
    except Exception as pre_exc:
      append_event(
        "system", "weekend_preremine_skip", bridge_dir=bridge_dir, summary=str(pre_exc),
      )
  ...
  while not _stop.is_set():
    ...
    try:
      ...
      try:
        from mt5_bridge.protocol import history_replay_active
        feed_active = history_replay_active(bridge_dir)
      except Exception:
        feed_active = False
      if not feed_active:
        now_chk = time.time()
        if (now_chk - last_preremine_check) >= 60.0:
          ...
          try:
            from mt5_bridge.weekend_preremine import maybe_preremine_engines
            maybe_preremine_engines(_engines, bridge_dir=bridge_dir)
          except Exception as pre_exc:
            append_event(
...
def start_sim_worker(
  *,
  ...
  """Write sim_control.json for the Live EA. Decide loop = Live worker (same folder)."""
