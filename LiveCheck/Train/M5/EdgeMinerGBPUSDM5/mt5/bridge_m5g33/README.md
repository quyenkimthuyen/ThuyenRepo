...
Trong app: sidebar **MT5 Bridge** — switch **Live | Simulate** (cùng desk/chart/thống kê/sức khỏe; khác `bridge_m5g33/` vs `bridge_sim_m5g33/`)
...
- **Simulate EA (History Feed)** — ForgeBridge **v1.05** `InpMode = HISTORY_FEED`, `InpBridgeSubdir = bridge_sim_m5g33`; App ghi `sim_control.json` (from/to/delay); EA `CopyRates` gửi bar/fill như live
...
Lần đầu cần tự gắn vào chart GBPUSD M5:
...
Sau này khi sửa `mt5/Experts/ForgeBridgeM5G33.mq5`, chỉ chạy lệnh đầu tiên. Script
...
- Cần tài khoản **hedging**. Compile lại `ForgeBridgeM5G33` / `ForgeBridgeM5G33Sim` sau khi kéo code mới.
- Live Trade dashboard: KPI tổng + bảng từng model.

## History Feed (Simulate EA)

1. Biên dịch lại ForgeBridge **1.05**, gắn GBPUSD M5 (demo khuyến nghị).
2. Inputs: `InpMode = HISTORY_FEED`, `InpBridgeSubdir = bridge_sim_m5g33` (junction/Files giống live).
3. App **MT5 Bridge → Simulate EA**: chọn from/to + delay → Start feed.
4. Service quyết định chạy trên `mt5/bridge_sim_m5g33/` (không đụng `bridge_m5g33/` live).
