...
def _render_shared_model_bar(models: list[dict]) -> dict | None:
  ...
  Đổi tên / Archive / Export .tmpkg nằm ở tab **Tổng hợp** (chọn dòng trong bảng).
  """
  if not models:
    return None
  id_by_label = {format_model_label(m): m["id"] for m in models}
...
def _render_models_overview(models: list[dict], active: dict | None):
  ...
  st.caption(
    ...
    "Click 1 dòng → **Đổi tên** / **Archive** / **Export .tmpkg**. Active chọn ở dropdown phía trên."
  )
  ...
  if selected_idx is None or not (0 <= selected_idx < len(rows)):
    st.caption("Tip: click 1 dòng → Đổi tên / Archive / Export .tmpkg. Active chọn ở dropdown phía trên.")
  ...
  a1, a2, a3, a4 = st.columns([1, 1, 1.15, 1.85])
  with a1:
    if archived:
      if st.button(
        "Restore",
  ...
  with a3:
    if st.button(
      ...
      help=(
        ...
        "Ghi vào live/packages_inbox. Cần đã chạy Sức khỏe (có schedule.json)."
      ),
    ):
      try:
  ...
  with a4:
    if on_bridge and not archived:
      ...
      st.caption(
        "Export cần schedule từ tab **Sức khỏe**. "
        + (ready.get("error") or "")
      )
