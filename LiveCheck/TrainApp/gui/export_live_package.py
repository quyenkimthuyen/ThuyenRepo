...
def export_readiness(model: dict) -> dict[str, Any]:
  ...
  reasons: list[str] = []
  if sched_errs:
    reasons.append("thiếu schedule.json — chạy tab Sức khỏe (OOS remine)")
...
def export_model_tmpkg(
  ...
  *,
  ...
  if use_kb and kb_pin is None:
    raise RuntimeError(
      f"{mid}: use_kb nhưng thiếu kb_pin.json — mở tab Sức khỏe / tạo lại pin rồi export."
    )
  ...
  if sched_errs:
    raise RuntimeError(
      ...
      f"{'; '.join(sched_errs)}\n"
      "Chạy tab **Sức khỏe** (OOS remine) cho model này rồi Export lại."
    )
  ...
  if not payload.get("mining_search_space"):
    raise RuntimeError(
      ...
      "Chạy tab Sức khỏe / remine rồi Export lại."
    )
  lab = {
    "desk": desk_id,
    "instance": instance,
