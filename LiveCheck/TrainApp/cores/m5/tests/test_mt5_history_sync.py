...
def test_chunk_protocol_resumes_until_done(tmp_path, monkeypatch):
  ...
  assert request["action"] == "export_m5_history"
  assert request["period"] == "M5"
...
def test_history_request_uses_desk_symbol_and_period(tmp_path, monkeypatch):
  """BUG-01: GBP desk must request GBPUSD/M5, not hardcoded EURUSD."""
  _temp_store(tmp_path, monkeypatch)
  _use_desk(monkeypatch, "g33")
  bridge = tmp_path / "bridge"
...
def test_cache_path_tracks_desk_after_import(monkeypatch):
  ...
  monkeypatch.setenv("TRAINAPP_DESK", "g33")
  monkeypatch.setenv("TRAINAPP_ROOT", str(_TRAINAPP_ROOT))
  root = str(_TRAINAPP_ROOT)
  if root not in sys.path:
    sys.path.insert(0, root)
