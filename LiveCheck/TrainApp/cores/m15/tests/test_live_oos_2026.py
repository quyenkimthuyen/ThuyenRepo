...
def _tip_bar_ts(frame: pd.DataFrame, week: str):
  ...
  tip = week_start + pd.Timedelta(days=7) - pd.Timedelta(minutes=15)
...
def mt5_frame() -> pd.DataFrame:
  path = ROOT / "data" / "mt5_eurusd_m15.parquet"
  if not path.exists():
...
def test_live_oos_strategy_parity_2026(live_2026_parity):
  ...
  for m in r["oos_mismatches"]:
    week = m["week"]
    tip = pd.Timestamp(week) + pd.Timedelta(days=7) - pd.Timedelta(minutes=15)
...
if __name__ == "__main__":
  frame = _normalize(pd.read_parquet(ROOT / "data" / "mt5_eurusd_m15.parquet"))
  model = get_model_by_id(MODEL_ID)
  report = load_model_report(MODEL_ID)
  import tempfile
