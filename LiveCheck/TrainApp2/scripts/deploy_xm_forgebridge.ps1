...
$BridgeSubdirSim = [string]$yaml["bridge_sim_subdir"]
...
if (-not $BridgeSubdirSim) { $BridgeSubdirSim = "bridge_sim_$($InstanceId.ToLowerInvariant())" }
...
$EaNameSim = "ForgeBridge${InstanceId}Sim"
$SourceEaLive = Join-Path $RepoRoot "mt5\Experts\$EaNameLive.mq5"
$SourceEaSim = Join-Path $RepoRoot "mt5\Experts\$EaNameSim.mq5"
$ProjectBridge = Join-Path $RepoRoot "mt5\$BridgeSubdirLive"
$ProjectBridgeSim = Join-Path $RepoRoot "mt5\$BridgeSubdirSim"
$EaFolder = "EdgeMiner$InstanceId"
$EaMagicLive = if ($yaml["magic"]) { [int]$yaml["magic"] } else { 0 }
$EaMagicSim = if ($yaml["sim_magic"]) { [int]$yaml["sim_magic"] } else { 0 }
# One Live EA: history test is sim_control.json on the Live bridge (no Sim EX5).
...
function Select-AttachChart(
  ...
  Write-Host "No free $Symbol $TfLabel chart for $wantedName; creating a new chart (keep $otherName intact)."
...
if ($Attach) {
  if ($IsBoth) {
    ...
    Write-Step "Attach $EaName HISTORY_FEED to $Symbol $TfLabel"
