...
def _promote(desk: str, run: dict, *, max_quality: int = 4, max_high_r: int = 3) -> list[dict]:
  ...
  if _MODE in ("target60", "ooswalk") or desk == "g23" and _MODE in (
  ...
  if not q_hits and _MODE not in ("target60", "ooswalk"):
  ...
  r_hits = [] if _MODE == "target60" else [r for r in rows if _passes_r(r)]
  ...
  active_set = False
  cur_r = cur_wr = None
  if _MODE in ("refine", "fullera", "hybrid", "target60"):
    try:
  ...
  def _add(row: dict, *, track: str, set_active: bool) -> None:
    ...
    if _MODE == "ooswalk":
      ...
      if of and ot:
        oos_tag = f" OOS{of}→{ot}"
    prefix = "OOSW" if _MODE == "ooswalk" else ("M15Q" if track == "quality" else "M15R")
...
def run_desk(
  ...
  *,
  ...
  if mode in ("boost", "explore", "refine", "fullera", "hybrid") and desk != "g23":
  ...
  salvaged = _promote_missed_from_latest(desk) if salvage and mode != "ooswalk" else []
...
def main() -> int:
  ...
  ap.add_argument("--mode", choices=(
    "densify", "boost", "explore", "refine", "fullera", "hybrid", "target60", "ooswalk",
  ), default="densify",
                  help="ooswalk=era/OOS 6m walk; target60=WR>60+R>100; hybrid/boost/...=g23-only")
  ...
  if args.mode in ("boost", "explore", "refine", "fullera", "hybrid"):
