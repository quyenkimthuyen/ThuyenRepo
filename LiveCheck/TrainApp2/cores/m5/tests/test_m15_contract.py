...
def test_m15_constants_and_three_week_window():
  assert DEFAULT_TF == "M5"
  assert TRAIN_WEEKS == 3
  assert BARS_PER_WEEK == 2016
  # Continuous 5m tape: 3 calendar weeks = BARS_PER_WEEK * 3 bars
  index = pd.date_range("2026-01-01", periods=BARS_PER_WEEK * 3 + 100, freq="5min")
...
class _SignalMatrix:
  def __init__(self):
    first = pd.date_range("2026-07-15 07:00", periods=8, freq="5min")
    second = pd.date_range("2026-07-16 07:00", periods=8, freq="5min")
...
def test_signal_cap_respects_max_trades_per_broker_day():
  fm = _SignalMatrix()
  ...
  # 2 broker days × MAX_TRADES_PER_DAY (M5 denser cap)
  assert len(selected) == MAX_TRADES_PER_DAY * 2
  assert selected.to_series().groupby(selected.date).size().max() == MAX_TRADES_PER_DAY
...
def test_feature_schema_two_models_keep_legacy_runtime():
  ...
  assert current["feature_profile"] == "m5_parity"
