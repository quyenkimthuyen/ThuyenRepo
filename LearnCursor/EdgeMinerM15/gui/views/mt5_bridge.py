...
def _render_service_controls() -> None:
  ...
  with st.expander("Cấu hình service", expanded=not status.get("running")):
    ...
    st.caption(
      "Deploy **Live only**: compile/attach `ForgeBridgeM15` "
...
def _render_simulate_ea() -> None:
  ...
  st.caption(
    f"Deploy **Simulate only**: compile/attach `ForgeBridgeM15Sim`, "
    f"junction `{BRIDGE_SIM_DIR.name}`, chart M15 riêng (không đè Live)."
  )
  if st.button(
    "Deploy Simulate EA",
