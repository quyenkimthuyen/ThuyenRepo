"""Bridge decision engine — merge MT5 bars (M15 or H1) + weekly remine."""
...
from config import DEFAULT_RISK_PCT_PER_TRADE, MIN_TRAIN_BARS, get_active_tf
...
try:
  from mt5_bridge.models import coerce_instance_model_id
except ImportError:  # pragma: no cover - always defined, kept for safety
  def coerce_instance_model_id(model_id):
    return model_id
from mt5_bridge.protocol import DEFAULT_MODEL_ID, utc_now_iso
...
from runtime_profiles import RuntimeProfile, get_profile, get_tf_defaults
from signal_levels import project_signal_levels, week_bounds_for_ts
...
MT5_CACHE_PATH = cache_path_for("M15")
...
class BridgeEngine:
  ...
  def __init__(
    ...
    *,
    ...
    self.tf = str((profile.tf if profile else tf) or get_active_tf()).upper()
    self.defaults = get_tf_defaults(self.tf)
    self.bar_minutes = int(self.defaults.bar_minutes)
    ref_profile = profile or get_profile(self.tf, "live")
    try:
      self.model_id = coerce_instance_model_id(model_id, tf=self.tf) or model_id
    except TypeError:
      self.model_id = coerce_instance_model_id(model_id) or model_id
    self.risk_pct = float(risk_pct)
    self.magic = int(magic) if magic is not None else int(ref_profile.magic)
    self.mt5_cache = mt5_cache or self.defaults.cache_parquet
    self.bridge_dir = bridge_dir or (profile.bridge_dir if profile else None)
  ...
  def ensure_history(self, force: bool = False) -> pd.DataFrame:
    ...
    live_bridge_dir = get_profile(self.tf, "live").bridge_dir
    if force:
      start_history_sync(live_bridge_dir, force=True, tf=self.tf)
    if self.mt5_cache.exists():
      ...
      self._fm_key = None
      return self._df
    start_history_sync(live_bridge_dir, tf=self.tf)
    ...
    *,
    ...
    if scheduled and isinstance(scheduled.get("strategy"), dict):
      ...
      if ts_use < 0 or te_use <= ts_use or te_use > fm_mine.n:
        ts_fb, te_fb = get_train_window_indices(
          df_mine, week_start, train_window, train_unit=self.defaults.train_unit,
        )
    ...
    ts, te = get_train_window_indices(
      df_mine, week_start, train_window, train_unit=self.defaults.train_unit,
    )
  ...
  def merge_bar(self, bar: dict) -> pd.Timestamp:
    ...
    if self.mt5_cache.resolve() == self.defaults.cache_parquet.resolve():
  ...
  def decide_for_bar(self, bar: dict) -> dict:
    ...
    params = self._params
    train_unit = self.defaults.train_unit
    train_window = int(
      params.get("train_window")
      or params.get("train_months" if train_unit == "months" else "train_weeks")
      or self.defaults.train_length,
    )
    ...
    if (
      ...
      or self._model.get("data_timeframe") != self.tf
    ...
    week_start, week_end = week_bounds_for_ts(bar_ts)
    ...
    unit_tag = "m" if train_unit == "months" else "w"
    cache_key = (
      f"{week_start.date()}|{model_id}|{kb_profile}@{kb_snapshot}|{train_window}{unit_tag}|"
    ...
    if strat is None:
      ...
      df_chk = self.load()
      ts, te = get_train_window_indices(df_chk, week_start, train_window, train_unit=train_unit)
      reason = (
    ...
    # Cadence: day slots (M15) vs week slots (H1) — driven by TF defaults, not
    # a hardcoded branch, so a future TF only needs a new TfDefaults entry.
    week_cadence = self.defaults.max_trades_per_day is None and self.defaults.max_trades_per_week is not None
    if week_cadence:
      max_trades = int(getattr(strat, "max_trades_per_week", None) or self.defaults.max_trades_per_week or 0)
      if self.bridge_dir is not None:
        real_open, period_n = _journal_open_and_week_count(self.bridge_dir, week_start, week_end)
      else:
        period_n = len(week_trades)
        real_open = bool(open_position)
    else:
      broker_day = utc_to_broker_time(bar_ts).date()
      max_trades = int(getattr(strat, "max_trades_per_day", None) or self.defaults.max_trades_per_day or 0)
      if self.bridge_dir is not None:
        real_open, period_n = _journal_open_and_day_count(self.bridge_dir, broker_day)
      else:
        period_n = len(
          [t for t in week_trades if utc_to_broker_time(t.entry_time).date() == broker_day],
        )
        real_open = bool(open_position)
    slots_left = max(max_trades - period_n, 0)
    if real_open:
      decision = self._hold(
    ...
    proj = project_signal_levels(
      fm, strat, bar_idx, direction, spread, slip, bar_minutes=self.defaults.bar_minutes,
    )
    if not proj:
      decision = self._flat(
    ...
    expires = bar_ts + pd.Timedelta(minutes=self.defaults.bar_minutes)
    decision = {
      "signal_id": sig_id,
      "action": action,
      "entry": proj["entry_px"],
  ...
  def _flat(
    ...
    return {
      ...
      "bar_time": _fmt_bar(bar_ts),
      "model_id": model_id,
      "expires_bar_time": _fmt_bar(bar_ts + pd.Timedelta(minutes=self.defaults.bar_minutes)),
