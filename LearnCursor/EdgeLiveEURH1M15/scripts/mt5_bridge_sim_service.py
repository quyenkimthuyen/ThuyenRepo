...
Usage:
  ...
  python scripts/mt5_bridge_sim_service.py --tf H1 --from ... --to ...
"""
...
# --tf must be resolved *before* importing any TF-dependent module (config
# defaults, cache paths, bridge dirs, magics) so this process boots fully in
# that TF's context regardless of FORGE_TF / other services' active TF.
def _early_tf() -> str:
  for i, a in enumerate(sys.argv):
    if a == "--tf" and i + 1 < len(sys.argv):
      return set_active_tf(sys.argv[i + 1])
  env = os.environ.get("FORGE_TF", "").strip().upper()
  if env in ("H1", "M15"):
    return set_active_tf(env)
  return set_active_tf("M15")


_EARLY_TF = _early_tf()
_PROFILE = get_profile(_EARLY_TF, "sim")


def _register(pid_path: Path, log_path: Path, bridge_dir: Path, args) -> None:
...
def main() -> int:
  from config import set_active_tf
  from mt5_bridge.background import _run_sim_bridge_loop, sim_pid_path, sim_service_log_path
  ...
  ap.add_argument("--tf", choices=["M15", "H1"], default=_EARLY_TF)
  ...
  args = ap.parse_args()
  set_active_tf(args.tf)
  profile = get_profile(args.tf, "sim")
  if not args.bridge_dir:
    args.bridge_dir = str(profile.bridge_dir)

  pid_path = sim_pid_path(args.tf)
  log_path = sim_service_log_path(args.tf)
  bridge_dir = Path(args.bridge_dir)
  _register(pid_path, log_path, bridge_dir, args)
  ...
  bridge_thread = threading.Thread(
    ...
    args=(stop, args.model_id, float(args.risk_pct), args.tf),
    name=f"sim-bridge-decide-{args.tf}",
    daemon=True,
  )
  bridge_thread.start()
  ...
  try:
    print(
      f"[sim-service] tf={args.tf} pid={os.getpid()} from={args.date_from} to={args.date_to} "
      f"delay_ms={args.delay_ms} model={args.model_id} dir={bridge_dir}",
      flush=True,
    )
    ...
    traceback.print_exc()
    write_sim_state({"status": "error", "error": str(e), "service_pid": None}, bridge_dir)
    return 1
  finally:
    stop.set()
