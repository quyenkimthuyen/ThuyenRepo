...
from config import (
  BARS_PER_WEEK, MAX_TRADES_PER_DAY, MAX_TRADES_PER_WEEK,
  TARGET_TRADES_PER_WEEK, get_active_tf,
)
    ...
    for items in day_buckets.values():
      ...
      for score, direction, i in items:
        ...
        if all(abs(i - other) >= strat.min_bars_between for other in selected):
          signals[i] = direction
          selected.append(i)
  elif candidates and getattr(strat, "max_trades_per_week", 0) > 0:
    # H1 cadence: cap trades per ISO week
...
def backtest_mined(
  ...
  while i < end_idx - 1:
    ...
    if sig != 0:
      ...
      if (
        strat.max_trades_per_day > 0
        and entries_by_broker_day.get(broker_day, 0) >= strat.max_trades_per_day
      ):
        i += 1
        continue
                ...
                for thr in space.score_thresholds:
                  for min_match in space.min_rules_matches:
                    for ml_thr in space.ml_probability_thresholds:
                      ...
                      day_cap = int(d.max_trades_per_day or 0)
                      week_cap = int(d.max_trades_per_week or 0)
                      strat = MinedStrategy(
                        ...
                        max_trades_per_day=day_cap,
                        max_trades_per_week=week_cap,
                        ml_prob_min=ml_thr,
