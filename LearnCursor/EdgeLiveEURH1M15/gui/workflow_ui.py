...
def render_workflow_panel():
  ...
  if cur == 4 and not state.get("sim_started_at") and not state.get("paper_started_at"):
    if st.button("📡 Mở MT5 Bridge Simulate", use_container_width=True, key="wf_sim_start"):
      mark_simulate_started()
      _go("mt5_bridge")
...
def render_workflow_banner(*, page_step: int | None = None):
  """Banner ngắn trên Bridge (chỉ khi chưa xong bước đó)."""
  wf = assess_workflow()
  cur = wf["current_step"]
