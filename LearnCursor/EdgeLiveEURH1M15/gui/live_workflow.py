"""Quy trình live — huấn luyện KB → Grid → Trade Model → Live/Sim."""
...
WORKFLOW_STEPS: tuple[dict, ...] = (
  ...
  },
  {
    ...
    "detail": "Tạo **Trade Model** từ kết quả Grid Search — dùng cho Live/Sim & phân tích.",
    "nav_page": "learning",
    "learning_tab": "models",
  },
  {
    "id": 4,
    "key": "simulate",
    "title": "Simulate trên Bridge",
    "short": "④ Simulate",
    "subtitle": "HISTORY_FEED · đúng Trade Model",
    "detail": "Chạy Simulate (H1/M15) trên MT5 Bridge — so với backtest trước khi live.",
    "nav_page": "mt5_bridge",
    "learning_tab": None,
  },
  {
    ...
    "short": "⑤ Live",
    "subtitle": "Chỉ khi Simulate khớp backtest",
...
def load_workflow_state() -> dict:
  ...
  data.setdefault("sim_started_at", None)
  data.setdefault("paper_started_at", None)  # legacy alias
...
def mark_simulate_started():
  state = load_workflow_state()
  if not state.get("sim_started_at"):
    state["sim_started_at"] = datetime.now(timezone.utc).isoformat()
    save_workflow_state(state)


# Back-compat alias
mark_paper_started = mark_simulate_started
...
def assess_workflow(*, force: bool = False) -> dict:
  ...
  for spec in WORKFLOW_STEPS:
    ...
    if sid == 1:
      ...
      started = state.get("sim_started_at") or state.get("paper_started_at")
      if started:
        progress = f"Simulate từ {str(started)[:10]}"
      else:
        progress = "Chưa bắt đầu Simulate"
    else:
      done = step5_done
      progress = state.get("live_notes") or "Chỉ khi Simulate khớp backtest"
