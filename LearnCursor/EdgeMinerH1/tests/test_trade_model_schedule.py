"""Trade Model freeze schedule — hydrate + Bridge prefers schedule over remine."""
...
def test_strategy_roundtrip_preserves_genome():
  ...
  assert back.atr_mult_sl == 0.7
  assert back.trail_activate_r == 1.8
  assert abs(back.long_rules[0].threshold - 40.123456) < 1e-9
  assert abs(back.short_rules[0].weight - 2.25) < 1e-9


def test_lookup_prefers_oos_schedule_over_live(tmp_models_dir):
  mid = "tm_test_schedule"
  strat_oos = _sample_strat("oos #1111")
  strat_live = _sample_strat("live #2222")
  ...
  hit = lookup_week_strategy(mid, "2025-01-06")
...
def test_bridge_uses_schedule_without_remine(tmp_models_dir, monkeypatch):
  ...
  monkeypatch.setattr(eng_mod, "lookup_week_strategy", lookup_week_strategy)

  # Minimal fake frame large enough for train window checks
  ...
  def _fake_attach(strat_obj, *_a, **_k):
    return strat_obj

  monkeypatch.setattr(eng_mod, "attach_ml_scorer", _fake_attach)
  monkeypatch.setattr(
    eng_mod, "get_train_window_indices",
    lambda *_a, **_k: (10, 600),
  )
