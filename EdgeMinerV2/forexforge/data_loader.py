...
def _fetch_dukascopy_h1(start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
  ...
  if df.empty:
    raise RuntimeError(
      f"Failed to fetch EUR/USD H1 from Dukascopy ({start.date()} -> {end.date()})."
    )
...
def _save_cache(df: pd.DataFrame):
  ...
  try:
    ...
    get_logger("forexforge.data_loader").debug("notify_data_updated skipped: %s", exc)
...
def download_eurusd_h1(
  ...
  """
  Download EUR/USD H1 from Dukascopy (2022+ supported).
  Cached at data/eurusd_h1.parquet.
  """
  import sys
  try:
  ...
  if cached is not None and not force_refresh:
    # Prefer local parquet whenever it covers the request — avoid Dukascopy on every job.
    cover_end = end - pd.Timedelta(days=2)
    if cached.index[0] <= start and cached.index[-1] >= min(cover_end, cached.index[-1]):
      # If no end_date, use whatever cache has through its last bar
      if end_date is None:
        return cached[cached.index >= start].copy()
      if cached.index[-1] >= cover_end:
        return cached[(cached.index >= start) & (cached.index < end)].copy()
      # Partial cover: return cache slice and skip network unless force_refresh
      if cached.index[-1] > start:
        return cached[(cached.index >= start) & (cached.index < end)].copy()

  print(f"  Fetching Dukascopy EUR/USD H1: {start.date()} -> {end.date()} ...")
