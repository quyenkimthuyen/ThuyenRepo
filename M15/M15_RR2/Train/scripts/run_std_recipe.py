...
def _ensure_kb(
  ...
  *,
  ...
  from gui.era_compare import ensure_profile_learned, _profile_epoch_count
  ...
  loops = int(loops if loops is not None else (s.get("learning_loops") or 3))
  if not eras:
    missing = ",".join(era_keys or [])
    raise RuntimeError(f"{desk}: không có giai đoạn KB trong catalog ({missing or 'learning_era_keys trống'})")
  learned, skipped = [], []
  for era in eras:
    label = era.get("label") or era["kb_profile"]
    have = 0 if reset else _profile_epoch_count(era["kb_profile"])
    action = "RESET" if reset else ("continue" if have else "learn")
    _log(
      desk,
      f"KB {action} · {label} · {have}→{loops} epoch · {era['learn_from']}→{era['learn_until']}",
    )
    spec = {
...
def _run_grid(
  ...
  *,
  ...
  loops: int | None = None,
  oos_keys: list[str] | None = None,
  epochs: list[int] | None = None,
) -> dict:
  from gui.app_settings import get_settings, grid_build_kwargs
  from gui.grid_search_engine import (
    build_grid, build_grid_from_settings, grid_readiness, run_grid, save_grid_run,
  )
  from config import DEFAULT_TF

  s = get_settings()
  if mining_presets or era_keys or train_weeks or loops or oos_keys:
    s = dict(s)
    if mining_presets:
    ...
    if loops:
      s["learning_loops"] = int(loops)
    if oos_keys:
      s["oos_window_keys"] = list(oos_keys)
  ...
  if epochs:
    ...
    for pid in kw.get("kb_profiles") or []:
      (kw.setdefault("selected_epochs", {}) or {})[pid] = list(epochs)
    sig = kw.pop("settings_signature", None)
    era_cfg = kw.pop("learning_era_keys", [])
    oos_cfg = kw.pop("oos_window_keys", [])
    loops_cfg = kw.pop("learning_loops", loops)
    specs = build_grid(**kw)
    config = {
      **{k: v for k, v in kw.items()},
      "settings_signature": sig,
      "learning_era_keys": era_cfg,
      "oos_window_keys": oos_cfg,
      "learning_loops": loops_cfg,
      "source": "std_recipe",
      "selected_epochs_override": list(epochs),
    }
  else:
...
def _wipe_learn_artifacts(desk: str) -> None:
  """Xóa KB, grid, TM, compare/simulate cũ để học lại từ đầu."""
  import shutil

  rt = Path(os.environ["TRAINAPP_RUNTIME"])
  removed: list[str] = []
  # TM first — leftover pins would re-lock KB and skip RESET.
  for rel in ("results/trade_models",):
    p = rt / rel
    if p.exists():
      shutil.rmtree(p)
      removed.append(rel)
  for name in ("trade_models.json", "active_trade_model.json"):
    p = rt / "results" / name
    if p.exists():
      p.unlink()
      removed.append(name)
  try:
    from kb_profiles import list_profiles, unprotect_profile, wipe_unprotected_profiles

    for p in list_profiles():
      pid = str(p.get("id") or "").strip()
      if pid:
        unprotect_profile(pid)
    kb_out = wipe_unprotected_profiles()
    if kb_out.get("deleted"):
      removed.append("kb:" + ",".join(kb_out["deleted"]))
  except Exception as exc:
    _log(desk, f"KB API wipe skipped: {exc}")
  know = rt / "learning" / "knowledge.json"
  if know.exists():
    know.unlink()
    removed.append("knowledge.json")
  kb_dir = rt / "learning" / "kb_profiles"
  if kb_dir.exists():
    shutil.rmtree(kb_dir)
    removed.append("learning/kb_profiles")
  for rel in (
    "results/grid_search",
    ...
    if p.exists():
      shutil.rmtree(p)
      removed.append(rel)
  for name in ("learning_report.json",):
    p = rt / "results" / name
    if p.exists():
      p.unlink()
      removed.append(name)
...
def run_desk(
  ...
  *,
  ...
  s = get_settings()
  if persist_loops and loops:
    s = dict(s)
    s["learning_loops"] = max(1, min(12, int(loops)))
    save_settings(s)
    s = get_settings()
  grid_presets = list(mining_presets) if mining_presets else None
  use_loops = int(loops if loops is not None else (s.get("learning_loops") or 3))
  _log(
    desk,
    ...
    f"loops={use_loops} presets={grid_presets or s.get('mining_presets')} "
    f"oos={oos_keys or s.get('oos_window_keys')} {s.get('backtest_from')}→{s.get('backtest_to')} "
    f"spread={s.get('spread_pips')}/{s.get('slippage_pips')}",
  )
  kb = _ensure_kb(desk, reset=reset_kb, loops=use_loops, era_keys=era_keys)
  if learn_only:
    out = {"desk": desk, "kb": kb, "run_id": None, "n_combos": 0, "created": []}
    _log(desk, f"LEARN ONLY {json.dumps(out, ensure_ascii=False)}")
    return out
  run = _run_grid(
    desk, workers=workers, mining_presets=grid_presets,
    era_keys=era_keys, train_weeks=train_weeks,
    loops=use_loops, oos_keys=oos_keys, epochs=epochs,
  )
...
def main() -> int:
  ...
  ap.add_argument(
    ...
    help="Xóa hết KB (kể cả đã khóa), grid_search, trade model, compare/simulate trước khi học",
  )
  ap.add_argument(
  ...
  ap.add_argument(
    "--era-keys", default="",
    help="Override learning_era_keys cho học KB + grid lần này (csv). Không ghi đè Settings.",
  )
  ap.add_argument(
    "--weeks", default="",
  ...
  ap.add_argument(
    ...
    help="Số epoch học + grid (1–12). Không ghi đè Settings trừ khi --persist-loops.",
  )
  ap.add_argument(
    "--persist-loops", action="store_true",
    help="Ghi learning_loops vào app_settings (cùng --loops).",
  )
  ap.add_argument(
    "--oos-keys", default="",
    help="Override oos_window_keys cho grid lần này (csv). Không ghi đè Settings.",
  )
  ap.add_argument(
    "--epochs", default="",
    help="Chỉ grid các snapshot epoch này (csv, ví dụ 11,12). Học vẫn theo --loops.",
  )
  ap.add_argument(
    "--learn-only", action="store_true",
    help="Chỉ học KB (theo --era-keys/--loops), không chạy grid.",
  )
  args = ap.parse_args()
  ...
  oos_override = [k.strip() for k in args.oos_keys.split(",") if k.strip()] or None
  epoch_override = [int(e) for e in args.epochs.split(",") if e.strip()] or None
  loops_override = None
  if args.loops is not None:
    loops_override = max(1, min(12, int(args.loops)))
  summary = []
  rc = 0
  for desk in desks:
    try:
      if args.replay_only:
        ...
        summary.append(run_desk(
          ...
          oos_keys=oos_override,
          epochs=epoch_override,
          learn_only=bool(args.learn_only),
          persist_loops=bool(args.persist_loops),
        ))
    except Exception as exc:
      rc = 1
      try:
