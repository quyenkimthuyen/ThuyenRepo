...
Derived (example B4):
  ...
  EA              ForgeBridgeM15E21B4 / ForgeBridgeM15E21B4Sim
...
# Rename EA sources
if [[ -f "$DST_ROOT/mt5/Experts/ForgeBridgeM15E21.mq5" ]]; then
  mv "$DST_ROOT/mt5/Experts/ForgeBridgeM15E21.mq5" "$DST_ROOT/mt5/Experts/${EA_LIVE}.mq5"
fi
if [[ -f "$DST_ROOT/mt5/Experts/ForgeBridgeM15E21Sim.mq5" ]]; then
  mv "$DST_ROOT/mt5/Experts/ForgeBridgeM15E21Sim.mq5" "$DST_ROOT/mt5/Experts/${EA_SIM}.mq5"
fi
...
replace_file(
  ...
  [
    (r'^BRIDGE_DIR = ROOT / "mt5" / "bridge_m15e21"', f'BRIDGE_DIR = ROOT / "mt5" / "{bridge_live}"'),
    (r'^BRIDGE_SIM_DIR = ROOT / "mt5" / "bridge_sim_m15e21"', f'BRIDGE_SIM_DIR = ROOT / "mt5" / "{bridge_sim}"'),
    (r"^DEFAULT_MAGIC = \d+", f"DEFAULT_MAGIC = {magic_live}"),
    (r"^DEFAULT_SIM_MAGIC = \d+", f"DEFAULT_SIM_MAGIC = {magic_sim}"),
    (r'^INSTANCE_ID = "M15E21"', f'INSTANCE_ID = "{instance_id}"'),
  ],
)
...
# deploy script — placeholder rename to avoid ForgeBridgeM15E21 prefix clobbering Sim
deploy = root / "scripts" / "deploy_xm_forgebridge.ps1"
if deploy.exists():
  text = deploy.read_text(encoding="utf-8")
  text = text.replace("ForgeBridgeM15E21Sim", "@@EA_SIM@@")
  text = text.replace("ForgeBridgeM15E21", "@@EA_LIVE@@")
  ...
  text = text.replace('$BridgeSubdirLive = "bridge_m15e21"', f'$BridgeSubdirLive = "{bridge_live}"')
  text = text.replace('$BridgeSubdirSim = "bridge_sim_m15e21"', f'$BridgeSubdirSim = "{bridge_sim}"')
  text = re.sub(r'\$EaNameLive = "[^"]+"', f'$EaNameLive = "{ea_live}"', text)
  text = re.sub(r'\$EaNameSim = "[^"]+"', f'$EaNameSim = "{ea_sim}"', text)
  text = text.replace('$EaFolder = "EdgeMinerM15E21"', f'$EaFolder = "{repo_name}"')
...
# EA sources — use placeholders so ForgeBridgeM15E21 is not a prefix of the new Sim name.
for ea_path, subdir, magic, label in (
  ...
  text = text.replace("@@EA_LIVE@@", ea_live)
  text = re.sub(
    r'const string INSTANCE_ID = "M15E21";',
...
text = text.replace('assert \'const string INSTANCE_ID = "M15E21"\' in m15', f'assert \'const string INSTANCE_ID = "{instance}"\' in m15')
text = text.replace('assert \'const string INSTANCE_ID = "M15E21"\' in m15_sim', f'assert \'const string INSTANCE_ID = "{instance}"\' in m15_sim')
...
text = text.replace('(M15_ROOT, "ForgeBridgeM15E21", "ForgeBridgeM15E21Sim")', f'(M15_ROOT, "{ea}", "{ea_sim}")')
text = re.sub(
  r'm15 = \(M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM15E21\.mq5"\)',
  f'm15 = (M15_ROOT / "mt5" / "Experts" / "{ea}.mq5")',
  text,
)
text = re.sub(
  r'm15_sim = \(M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM15E21Sim\.mq5"\)',
