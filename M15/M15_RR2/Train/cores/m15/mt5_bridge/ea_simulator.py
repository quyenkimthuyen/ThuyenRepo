  ...
  *,
  ...
  while True:
    ...
    if not ea_acked and (now - t0) >= timeout_sec:
      ...
      ea_hint = f"ForgeBridge{INSTANCE_ID}"
      msg = (
        f"EA không phản hồi sim_control trong {timeout_sec:.0f}s — "
        f"gắn {ea_hint} lên chart (Live mode), đúng folder bridge, "
        "rồi Deploy lại nếu cần."
      )
