...
from analytics import equity_series, monthly_breakdown, trades_json_to_df, weekly_breakdown, daily_breakdown
...
def build_daily_series_figure(
  ...
  *,
  ...
  """Single-source daily R chart (calendar day)."""
  return build_period_series_figure(
    daily,
    title=title,
    series_name=series_name,
...
def compare_live_week_to_oos(
  ...
  *,
  ...
  if not report:
    ...
    out["message"] = (
      f"Chưa có report Health OOS — chạy Trade Models → {LABEL_TAB_OOS} (KB ON) rồi đối chiếu."
    )
    return out
...
def build_multi_model_period_figure(
  ...
  *,
  ...
  if n_ticks > 6:
    fig.update_xaxes(tickangle=-45 if n_ticks > 14 else -35, row=2, col=1)
  return fig
...
def build_multi_model_daily_figure(
  daily_by_model: dict[str, pd.DataFrame],
  *,
  title: str = "R theo ngày · Compare Trade",
) -> go.Figure | None:
  """Grouped daily total_r bars + multi-line cum R (calendar day)."""
  return build_multi_model_period_figure(
    daily_by_model,
    title=title,
    period_col="day",
...
def build_multi_model_price_figure(
  ...
  *,
  ...
  if title is None:
    ...
    title = f"Giá {tf_label()} · lệnh theo model"
