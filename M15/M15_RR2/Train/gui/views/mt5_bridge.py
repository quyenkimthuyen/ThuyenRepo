...
def _render_live_reward_all(
  ...
  *,
  ...
  """Overlay daily / weekly / monthly / equity for every roster model."""
  ...
  td, tw, th, tr = st.tabs([
    LABEL_CHART_DAILY, LABEL_CHART_WEEKLY, LABEL_CHART_MONTHLY, LABEL_CHART_EQUITY,
  ])
  with td:
    fig_d = build_multi_model_daily_figure(
      daily_by, title="Ngày · tất cả model · Live Auto",
    )
    if fig_d:
      show_plotly(fig_d, "Ngày · tất cả model")
      st.caption("Chỉ hiện **ngày có lệnh**. Bar = R ngày đó, đường dưới = R tích lũy.")
    else:
      st.info("Chưa có chuỗi ngày Live.")
  with tw:
...
def _render_model_monitor_body() -> None:
  ...
  if source == "live":
    with st.expander(f"{live_label} · Bridge auto", expanded=True):
      ...
      with td:
        live_daily_title = f"Ngày · {live_label} · {bundle['model_label']}"
        fig_d = build_daily_series_figure(
          bundle["live"].get("daily"),
          title=live_daily_title,
          series_name=live_label,
          color=LIVE_SERIES_COLOR,
        )
        if fig_d:
          show_plotly(fig_d, live_daily_title)
          st.caption("Chỉ hiện **ngày có lệnh**. Bar = R ngày đó, đường dưới = R tích lũy.")
        else:
          st.info(f"Chưa có chuỗi ngày {live_label}.")
      with tw:
