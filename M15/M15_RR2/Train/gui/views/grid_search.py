...
def _on_grid_oos_changed() -> None:
  persist_widget("gs_oos_labels", "grid.oos_labels")
  s = get_settings()
  option_to_key = {
    oos_window_option(w): w["key"] for w in get_oos_window_catalog(s)
  }
  keys = [
    option_to_key[opt]
    for opt in (st.session_state.get("gs_oos_labels") or [])
    if opt in option_to_key
  ]
  if keys:
    update_settings(oos_window_keys=keys)


def _sync_grid_oos_picker(settings: dict, *, disabled: bool = False) -> dict:
  """Multiselect OOS for this Grid run; persist to Settings oos_window_keys."""
  catalog = get_oos_window_catalog(settings)
  options = [oos_window_option(w) for w in catalog]
  option_to_key = {oos_window_option(w): w["key"] for w in catalog}
  saved_keys = list(settings.get("oos_window_keys") or [])
  default_labels = [opt for opt in options if option_to_key.get(opt) in saved_keys]
  restore_widget(
    "gs_oos_labels",
    default_labels or options[:1],
    preference_key="grid.oos_labels",
    options=options,
    multiple=True,
  )
  picked = [
    opt for opt in (st.session_state.get("gs_oos_labels") or [])
    if opt in option_to_key
  ]
  aligned = picked or default_labels or options[:1]
  if st.session_state.get("gs_oos_labels") != aligned:
    st.session_state["gs_oos_labels"] = aligned

  st.multiselect(
    "Chạy Grid theo cửa sổ OOS",
    options,
    key="gs_oos_labels",
    disabled=disabled,
    on_change=_on_grid_oos_changed,
    help=(
      "Chọn một hoặc nhiều cửa sổ đã có trong Cài đặt. "
      "Combo KB trùng ngày với OOS sẽ bị bỏ. Thêm/xóa cửa sổ ở ① Cài đặt."
    ),
  )
  keys = [
    option_to_key[opt]
  ...
  if not keys:
    st.warning("Chọn ít nhất một cửa sổ OOS để chạy Grid.")
    return settings
  if keys != list(saved_keys) and set(keys) != set(saved_keys):
    settings = update_settings(oos_window_keys=keys)
  pairs = describe_kb_oos_pairs(settings)
...
def render(embedded: bool = False):
  ...
  skipped_overlap = readiness.get("skipped_overlap") or []
  if kb_done and expected == 0 and skipped_overlap:
    st.error(
      "**Không combo Grid** — mọi cặp KB × OOS trùng thời gian học. "
      "Đổi giai đoạn học hoặc cửa sổ OOS trong Cài đặt."
    )
    for m in skipped_overlap:
      st.markdown(
        f"- **{m['label']}** (`{m['id']}`) trùng OOS "
        f"**{m.get('oos_label') or (m['oos_from'] + ' → ' + m['oos_to'])}**"
      )
  elif skipped_overlap:
    st.caption(
      "Grid bỏ combo KB trùng OOS: "
      + "; ".join(
        f"{m['label']} × {m.get('oos_label') or m['oos_from']}"
        for m in skipped_overlap
      )
    )
  ...
  gs_settings = _sync_grid_oos_picker(get_settings(), disabled=running)
  oos_ready = bool(st.session_state.get("gs_oos_labels"))
  specs, grid_config = build_grid_from_settings(gs_settings)
  grid_kw = {
  ...
  run_btn = False
  run_disabled = running or not kb_done or not oos_ready
  if new_specs:
    c1, c2 = st.columns(2)
    with c1:
      run_btn = st.button(
