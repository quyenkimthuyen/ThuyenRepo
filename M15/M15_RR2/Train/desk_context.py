...
def _relocate_under_root(raw: str, *, root: Path) -> str | None:
  ...
  try:
    import sys
    core = root.parent / "cores" / "m15"
    if str(core) not in sys.path:
      sys.path.insert(0, str(core))
    from app_paths import relocate_under_root
    abs_p = relocate_under_root(text, root=root)
    if abs_p is None:
      return None
  ...
  p = Path(text)
  norm = text.replace("\\", "/")
  if not p.is_absolute() and not (len(norm) >= 2 and norm[1] == ":"):
    ...
    for i, part in enumerate(parts):
      if part.lower() == "mt5" and i + 1 < len(parts) and parts[i + 1].lower().startswith("bridge"):
        bridge = parts[i + 1]
        break
    if not bridge:
      for part in reversed(parts):
        if part.lower().startswith("bridge"):
          ...
          break
    if bridge:
      abs_p = (root / "mt5" / bridge).resolve()
    elif "simulate_runs" in parts_l:
...
def apply_desk_env(desk_id: str) -> dict[str, Any]:
  ...
  try:
    ...
    ensure_live_autostart_after_move(cfg["id"])
  except Exception:
    pass
  return cfg
