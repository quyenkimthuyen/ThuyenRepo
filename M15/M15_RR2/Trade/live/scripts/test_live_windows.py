...
def check_constants() -> str:
  ...
  assert CLONE_TAG == "RR2", CLONE_TAG
  assert LIVE_APP_PORT == 9601, LIVE_APP_PORT
  assert LIVE_BRIDGE_PORT == 10601, LIVE_BRIDGE_PORT
  assert LIVE_MAGIC_BASE == 20285201, LIVE_MAGIC_BASE
  assert LIVE_SIM_MAGIC_BASE == 20286201, LIVE_SIM_MAGIC_BASE
  assert LIVE_INSTANCE_ID == "LIVERR2"
  assert LIVE_EA_STEM == "ForgeBridgeLiveRR2"
