...
$CloneRoot = (Resolve-Path (Join-Path $RepoRoot "..")).Path
$CloneTag = ([IO.Path]::GetFileName($CloneRoot) -replace '^M15_','')
$TagLower = $CloneTag.ToLowerInvariant()
$SourceEaLive = Join-Path $RepoRoot ("mt5\Experts\ForgeBridgeLive{0}.mq5" -f $CloneTag)
$SourceEaSim = Join-Path $RepoRoot ("mt5\Experts\ForgeBridgeLiveSim{0}.mq5" -f $CloneTag)
$ProjectBridge = Join-Path $RepoRoot ("mt5\bridge_{0}" -f $TagLower)
$ProjectBridgeSim = Join-Path $RepoRoot ("mt5\bridge_sim_{0}" -f $TagLower)
$BridgeSubdirLive = "bridge_$TagLower"
$BridgeSubdirSim = "bridge_sim_$TagLower"
$EaNameLive = "ForgeBridgeLive$CloneTag"
$EaNameSim = "ForgeBridgeLiveSim$CloneTag"
$EaFolder = "EdgeMiner$CloneTag"
$EaMagicLive = 20285201
$EaMagicSim = 20286201
$IsBoth = ($Mode -eq "Both")
  ...
  foreach ($row in $enabled) {
    ...
    if (-not $map.ContainsKey($key)) {
      ...
      $entry = New-Object psobject -Property @{
        ...
        Timeframe = $tf
        PeriodSize = $period
        BridgeSubdir = ("{0}_{1}" -f $BridgeSubdirLive, $key)
        ProjectDir = Join-Path $RepoRoot ("mt5\{0}_{1}" -f $BridgeSubdirLive, $key)
        Magic = $magic
        RiskPct = $risk
