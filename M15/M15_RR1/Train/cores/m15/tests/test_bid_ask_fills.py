...
def test_pips_to_points():
  assert pips_to_points(0) == 0
  assert pips_to_points(1.0) == 10
  assert pips_to_points(0.5) == 5
  assert pips_to_points(-1) == 0
  assert pips_to_points(None) == 0
...
def test_partial_runner_trail_keeps_more_r_than_be_park():
  n = 8
  atr = 0.001
  fm = _bt_fm(n)
  fm.atr[:] = atr
  ...
  fm.high[:] = 1.10005
  fm.low[:] = 1.09980
  fm.high[2] = 1.10 + 0.90 * atr
  fm.low[2] = 1.10 + 0.70 * atr
  fm.high[3] = 1.10 + 1.70 * atr
  fm.low[3] = 1.10 + 1.40 * atr
  fm.high[4] = 1.10 + 0.50 * atr
  fm.low[4] = 1.10 + 0.20 * atr
  signals = np.zeros(n, dtype=np.int8)
  signals[0] = 1
  kw = dict(
    atr_mult_sl=1.0, rr_ratio=2.2, max_hold_bars=96, max_trades_per_day=4,
    session_filter=False, min_bars_between=1, exit_mode="partial",
    anti_chase=False, confirm_r=0.0, partial_pct=0.70, partial_at_r=0.90,
    tp_ignores_spread_buffer=True,
  )
  park = backtest_mined(
    fm, MinedStrategy(**kw, partial_runner_trail_r=0.0),
    signals, 0, n, spread_pips=0.0, slippage_pips=0.0,
  )
  trail = backtest_mined(
    fm, MinedStrategy(**kw, partial_runner_trail_r=0.40),
    signals, 0, n, spread_pips=0.0, slippage_pips=0.0,
  )
  assert len(park) == 1 and len(trail) == 1
  assert park[0].r_multiple > 0
  assert trail[0].r_multiple > park[0].r_multiple
