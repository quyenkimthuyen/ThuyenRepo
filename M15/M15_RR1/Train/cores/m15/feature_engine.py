...
class FeatureMatrix:
  ...
  def _build(self):
    ...
    # Expanding std through bar i — never the full-file std (that leaks OOS
    # vol into earlier bars; Grid would "see" June while deciding January).
    roc = pd.Series(c).pct_change(p.roc_period) * 100
    roc_std = roc.expanding(min_periods=max(p.roc_period, 20)).std()
    self.features["roc_5"] = (roc / (roc_std.replace(0, np.nan) + 1e-12)).values
