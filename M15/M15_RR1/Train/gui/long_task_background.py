...
JOB_LABELS = {
  ...
  "model_health": "Đánh giá OOS (KB ON/OFF)",
  "remine_health": "Đánh giá Remine ON/OFF",
...
def _worker_kb_then_grid(state: dict):
  ...
  if not specs:
    raise RuntimeError(
      "Không có combo Grid — kiểm tra Cài đặt (train / era / mining / KB trùng OOS)."
    )
...
def _worker_compare_trade(state: dict):
  ...
  extra_spread = float(p.get("additional_spread_pips") or 0.0)
  if len(model_ids) < 2:
