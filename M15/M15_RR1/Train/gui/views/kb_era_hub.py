...
def _tab_learn():
  ...
  with c2:
    ...
    epochs = st.number_input(
      "Số vòng học", 1, 16, key="hub_epochs",
  ...
  locked_reset = bool(pid_learn) and is_profile_protected(pid_learn) and bool(reset)
  if locked_reset:
    st.error(
      f"KB **{pid_learn}** đã khóa — không reset. "
      "Tắt Reset để học thêm epoch, hoặc bỏ khóa ở tab Danh sách."
    )
  elif pid_learn and is_profile_protected(pid_learn):
    st.info(
      f"KB **{pid_learn}** đã khóa — học thêm epoch được, snapshot Trade Model giữ nguyên."
    )
  ...
  if st.button(
    ...
    type="primary",
    key="hub_learn_run",
    disabled=running or locked_reset,
  ):
    try:
      start_job(
        "learning",
