...
OBJECTIVES = {
  ...
  "hit80": "RR=1 bar: WR>80% và Total R>80 (OOS)",
}
...
def _learn_span_for_profile(
  pid: str,
  kb_learn_by_profile: dict[str, tuple[str, str]] | None = None,
) -> tuple[str, str] | None:
  if kb_learn_by_profile and pid in kb_learn_by_profile:
    span = kb_learn_by_profile[pid]
    return str(span[0])[:10], str(span[1])[:10]
  try:
    from gui.app_settings import era_by_kb_profile
    era = era_by_kb_profile(pid)
  except Exception:
    era = None
  if era and era.get("learn_from") and era.get("learn_until"):
    return str(era["learn_from"])[:10], str(era["learn_until"])[:10]
  return None
...
def build_grid(
  *,
  ...
  oos_from: str,
  oos_to: str,
  oos_by_profile: dict[str, tuple[str, str]] | None = None,
  oos_windows: list[tuple[str, str]] | None = None,
  kb_learn_by_profile: dict[str, tuple[str, str]] | None = None,
  skip_kb_oos_overlap: bool = True,
  spread_pips: float = DEFAULT_SPREAD_PIPS,
  slippage_pips: float = DEFAULT_SLIPPAGE_PIPS,
  max_runs: int = 60,
  ...
  When ``oos_windows`` is set, every window is scored for each KB unless the
  learn span overlaps that OOS (inclusive dates). ``oos_by_profile`` only
  applies when ``oos_windows`` is empty (legacy CLI).
  """
  from gui.app_settings import kb_learn_overlaps_oos
  ...
  if oos_windows:
    windows = [(str(a)[:10], str(b)[:10]) for a, b in oos_windows]
  else:
    windows = [(str(oos_from)[:10], str(oos_to)[:10])]
      ...
      for win_from, win_to in windows:
        if include_kb_off:
          specs.append(GridSpec(
            ...
            oos_from=win_from, oos_to=win_to,
        ...
        for pid in kb_profiles:
          if oos_windows:
            spec_oos_from, spec_oos_to = win_from, win_to
          else:
            pf_oos = (oos_by_profile or {}).get(pid)
            spec_oos_from = pf_oos[0] if pf_oos else win_from
            spec_oos_to = pf_oos[1] if pf_oos else win_to
          if skip_kb_oos_overlap:
            span = _learn_span_for_profile(pid, kb_learn_by_profile)
            if span and kb_learn_overlaps_oos(span[0], span[1], spec_oos_from, spec_oos_to):
              continue
          base = dict(
            oos_from=spec_oos_from,
            oos_to=spec_oos_to,
            spread_pips=spread_pips,
            slippage_pips=slippage_pips,
          )
          ok, _ = kb_valid_for_backtest(pid, spec_oos_from, spec_oos_to)
          if not ok:
            continue
...
_BUILD_GRID_META_KEYS = {
  "settings_signature", "learning_era_keys", "learning_loops",
  "oos_window_keys", "source", "seed_rows",
}


def estimate_grid_count(**kwargs) -> tuple[int, int]:
  """Returns (count, capped_count)."""
  kwargs = {k: v for k, v in kwargs.items() if k not in _BUILD_GRID_META_KEYS}
...
def expected_grid_count_from_settings(settings: dict | None = None) -> int:
  ...
  from gui.app_settings import count_valid_kb_oos_slots, grid_build_kwargs, resolve_oos_windows
  ...
  windows = kw.get("oos_windows") or resolve_oos_windows(settings)
  n_win = len(windows) if windows else 1
  valid_kb = count_valid_kb_oos_slots(settings)
  if kw.get("include_kb_off"):
    return trains * (n_win + valid_kb * loops) * mining_n
  return trains * valid_kb * loops * mining_n
  ...
  skipped_overlap = [
    {
      "id": row["kb_profile"],
      "label": row["era_label"],
      "oos_from": row["oos_from"],
      "oos_to": row["oos_to"],
      "oos_label": row["oos_label"],
    }
    for row in describe_kb_oos_pairs(settings)
    if row["overlap"]
  ]
...
def _score(row: dict, objective: str) -> float:
  ...
  if objective == "hit80":
    ...
    wr = float(row.get("win_rate_pct") or 0)
    n = int(row.get("n_trades") or 0)
    if wr > 80.0 and r > 80.0 and n >= 15:
      return wr * 10.0 + r + min(n, 160) * 0.05
    return wr + r * 0.15 + min(n, 160) * 0.08 - 1e6
  return float(row.get("total_r") or 0)
...
def run_single(spec: GridSpec) -> dict:
  ...
  if spec.use_kb and spec.kb_profile:
    from gui.app_settings import kb_learn_overlaps_oos
    span = _learn_span_for_profile(spec.kb_profile)
    if span and kb_learn_overlaps_oos(span[0], span[1], spec.oos_from, spec.oos_to):
      return {
        "key": spec.key(),
        "label": spec.label(),
        ...
        "n_trades": 0,
        "error": "KB học trùng giai đoạn OOS — bỏ combo.",
      }
...
def summarize_grid_config(config: dict | None) -> dict:
  ...
  windows = cfg.get("oos_windows")
  if windows:
    oos_txt = " · ".join(
      f"{str(a)[:10]} → {str(b)[:10]}" for a, b in windows
    )
  else:
...
def build_grid_from_settings(settings: dict | None = None) -> tuple[list[GridSpec], dict]:
  ...
  oos_window_keys = kwargs.pop("oos_window_keys", [])
  ...
  config = {
    **{k: v for k, v in kwargs.items()},
    ...
    "learning_era_keys": era_keys,
    "oos_window_keys": oos_window_keys,
    "learning_loops": loops,
    "source": "app_settings",
  }
  return specs, config
