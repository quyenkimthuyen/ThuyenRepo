...
def _passes(row: dict) -> bool:
  ...
  wr_gt, r_gt, n_ge = FILTER["wr_gt"], FILTER["total_r_gt"], FILTER["n_ge"]
  try:
    from gui.app_settings import get_settings
    s = get_settings()
    presets = [str(p) for p in (s.get("mining_presets") or [])]
    if str(s.get("grid_objective") or "") == "hit80" or any("hit80" in p for p in presets):
      wr_gt, r_gt, n_ge = 80.0, 80.0, 15
  except Exception:
    pass
  if not (wr > wr_gt and tot > r_gt and n >= n_ge and dd < FILTER["max_dd_lt"]):
    return False
...
def _ensure_kb(
  ...
  loops_n = int(s.get("learning_loops") or 3)
  learned, skipped = [], []
  for era in eras:
    label = era.get("label") or era["kb_profile"]
    _log(desk, f"KB {'RESET' if reset else 'ensure'} · {label} · {loops_n} epoch · {era['learn_from']}→{era['learn_until']}")
    spec = {
    ...
    t0 = time.time()
    out = ensure_profile_learned(spec, epochs=loops_n, reset=reset)
  ...
  return {"learned": learned, "skipped": skipped, "loops": loops_n}
...
def _run_grid(
  ...
  *,
  ...
  s = get_settings()
  if mining_presets or era_keys or train_weeks or loops:
    s = dict(s)
    if mining_presets:
...
def run_desk(
  ...
  *,
  ...
  if keep_settings and (era_keys or loops):
    s_save = dict(get_settings())
    if era_keys:
      s_save["learning_era_keys"] = list(era_keys)
    if loops:
      s_save["learning_loops"] = int(loops)
    save_settings(_sanitize_settings(s_save))
  s = get_settings()
  grid_presets = list(mining_presets) if mining_presets else None
  loops_n = int(loops) if loops else int(s.get("learning_loops") or 10)
  _log(
    desk,
    ...
    f"loops={loops_n} presets={grid_presets or s.get('mining_presets')} "
    f"oos={s.get('oos_window_keys')} {s.get('backtest_from')}→{s.get('backtest_to')} "
    f"spread={s.get('spread_pips')}/{s.get('slippage_pips')}",
  )
  kb = _ensure_kb(desk, reset=reset_kb, era_keys=era_keys, loops=loops_n)
  run = _run_grid(
    desk, workers=workers, mining_presets=grid_presets,
    era_keys=era_keys, train_weeks=train_weeks, loops=loops_n,
  )
...
def main() -> int:
  ...
  ap.add_argument(
    ...
    help="Override learning_loops (epoch) lần này. 0 = dùng Settings.",
  )
  args = ap.parse_args()
  ...
  loops_override = int(args.loops) if int(args.loops or 0) > 0 else None
  summary = []
  rc = 0
  for desk in desks:
    try:
