...
def main():
  ...
  # Compare app
  active_path = ROOT / "results" / "active_trade_model.json"
  active_id = (
    json.loads(active_path.read_text(encoding="utf-8")).get("id")
    if active_path.exists() else None
  )
  app_path = ROOT / "results" / "trade_models" / f"{active_id}.json"
  if app_path.exists():
