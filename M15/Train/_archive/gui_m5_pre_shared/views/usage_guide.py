...
"""
  ...
  st.markdown(
    f"""
### Compare Trade
So 1–N model trên cache M5 (`{d['cache']}`). Không EA. Kết quả = sim fills — **không** mở lệnh tài khoản.
...
4. Mỗi M5: EA `bar.json` → App decide → `decision.json` → BUY/SELL/FLAT → `fill.json` → `trades.json`.
