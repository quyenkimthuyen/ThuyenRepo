#!/usr/bin/env python3
"""Học KB 2024-h1/h2 + 2025-q4 (giữ 2025-h2/full), grid ss_vol/tight/dense trên OOS 2026. Không promote."""
...
# Already learned — do not reset. Re-grid vs new presets.
KEEP_GRID = ["era_2025_h2"]
PREV = "ss_vol 6w h2 ep1 WR50 +36.8R n=42 PF2.65"
...
def main() -> int:
  cfg = bind()
  log(f"more-models start pair={cfg.get('pair')} · keep 2025-h2/full · {PREV}")
  ...
  weeks = [6, 8]
  presets = ["gbp_fill_ss_vol", "gbp_fill_ss_tight", "gbp_fill_ss_dense"]
  pids = [e["kb_profile"] for e in LEARN_ERAS] + KEEP_GRID
  specs = build_grid(
    train_weeks=weeks,
    kb_profiles=pids,
  ...
  log("MORE MODELS DONE (no promote, no Active, 2025-h2/full untouched)")
