...
class KnowledgeBase:
  ...
  def rule_weight_multiplier(self, r) -> float:
    """Nhân weight rule dựa trên kinh nghiệm lịch sử."""
  ...
  def record_rule_outcomes(self, matched_keys: list[str], r_multiple: float):
    won = r_multiple > 0
  ...
  def add_ml_sample(self, features: dict[str, float], long_win: int, short_win: int):
