...
def render_era_compare():
  ...
  st.caption(
    ...
    "Ghép giai đoạn tại **Học → Huấn luyện bộ nhớ**."
  )
...
def render_train_window():
  ...
  from gui.train_window_compare import (
    DEFAULT_SPEC, train_month_options, best_combo, load_train_window_cache,
    metrics_table, result_key,
  )
  from gui.workspace import get_active_workspace

  st.subheader("So sánh cửa sổ học (theo Cài đặt)")
  st.caption(
    "So **có bộ nhớ vs không bộ nhớ** cho từng độ dài train đã chọn trong **Cài đặt**."
  )
  ...
  with c2:
    oos_from, oos_to = oos_period_inputs(
      "tw", kb_profile,
      default_from=ws.get("oos_from") or spec["oos_from"],
      default_to=ws.get("oos_to") or spec["oos_to"],
    )
  ...
  months = train_sel or (cache or {}).get("train_months_list") or train_month_options()
