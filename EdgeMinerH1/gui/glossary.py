...
HELP = {
  ...
  "train_weeks": "Số tuần dữ liệu gần nhất để học (tham chiếu M15).",
...
CONSTRAINT_LABELS = {
  ...
  "trades_per_week_target": "Mục tiêu ~2 lệnh/tuần",
}
...
def glossary_sections() -> list[tuple[str, list[tuple[str, str]]]]:
  """Thuật ngữ & viết tắt dùng trong app (H1)."""
  return [
    ("Học & kiểm chứng", [
      ("OOS / Kiểm chứng", "Giai đoạn chỉ test lệnh, không tối ưu lại chiến lược."),
      ("Học N tháng", "Mỗi tuần lấy N tháng data gần nhất để tìm chiến lược (3/6/9)."),
      ...
      ("Giai đoạn (era)", "Profile KB gắn khoảng thời gian, vd. era_2024_2025."),
    ...
    ("Vận hành MT5", [
      ...
      ("EA", "Expert Advisor trên MT5 (`ForgeBridgeH1` / `ForgeBridgeH1Sim`)."),
