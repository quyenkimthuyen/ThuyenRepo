...
def main() -> int:
  ...
  if (
    ...
    raise RuntimeError("MT5 H1 history sync is not complete.")
  ...
  if not valid:
    raise RuntimeError("Grid Search produced no profitable H1 result at 7-10 trades/week.")
  run_id = save_grid_run(
    rows,
    config={**config, "data_source": "mt5_ea", "timeframe": "H1"},
    objective=objective,
  )
  model = create_trade_model(
    valid[0], run_id=run_id, label="H1 Best", set_active=True, build_report=False,
  )
