#!/usr/bin/env python3
"""Backfill / export Trade Model freeze schedule from Health OOS config.

Re-runs walk-forward with the model's stored parameters and writes
``results/trade_models/{model_id}_schedule.json``.
"""
...
def main() -> int:
  ap = argparse.ArgumentParser(description="Export Trade Model OOS schedule")
  ap.add_argument("--model-id", default="tm_mt5_best_94ef551a")
  ...
  train_months = int(
    (model or {}).get("train_months")
    or cfg.get("train_months")
    or 9
  )
  ...
  if kb_snapshot in (None, "latest"):
    ...
    try:
      ...
      kb_snapshot = None
  oos_from = (model or {}).get("oos_from") or cfg.get("oos_from") or "2025-01-01"
  ...
  print(
    f"Export schedule {args.model_id} | train={train_months}m "
  ...
  # Compare names vs existing report weekly_log when present
  old_log = {
  ...
  # Smoke: path exists and has weekly genomes
