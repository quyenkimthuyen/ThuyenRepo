//+------------------------------------------------------------------+
//| ForgeBridgeH1Sim.mq5 — EdgeMiner H1 SIM (magic 20260727, bridge_sim_h1) |
...
//+------------------------------------------------------------------+
#property copyright "EdgeMinerH1 bridge SIM"
...
input string InpBridgeSubdir   = "bridge_sim_h1";    // under MQL5/Files/ (SIM HistoryFeed)
...
int OnInit()
{
   ...
   else
   {
      ...
      Print("ForgeBridgeH1 Live | Files/", InpBridgeSubdir, " | magic=", InpMagic);
   }
...
bool LoadHistoryRatesRange()
{
   ...
   if(StringLen(g_sim_to) <= 10)
      t_to += PeriodSeconds(PERIOD_H1) * 24 - 1;
   ...
   g_open_signal_id = "";
   g_had_position = false;
   Print("ForgeBridgeH1 HistoryFeed loaded bars=", g_hist_n,
...
bool PaperOpenFromDecision(const string json, const double entry_price, const string bar_time = "")
{
   ...
   WriteFillJsonEx("open", sid, action, true, "opened",
                   ...
                   false, "strategy", bt);
   Print("ForgeBridgeH1 HistoryFeed paper ", action, " @", entry_price,
...
void ProcessHistoryFeed()
{
   ...
   if(g_hist_n < 1 || g_hist_cursor >= g_hist_n)
   {
      ...
      g_sim_enabled = false;
      WriteSimControlFile();
      Print("ForgeBridgeH1 HistoryFeed completed bars=", g_hist_n);
      return;
   }
   ...
   if(flat && g_pending_decision == "")
   {
      ...
      if(WaitDecisionForBar(want, json, wait_ms))
      {
         ...
         Print("ForgeBridgeH1 HistoryFeed: no decision for ", want,
