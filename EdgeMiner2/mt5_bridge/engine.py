...
from data_loader import DATA_DIR, get_train_window_indices, get_week_indices, load_eurusd_h1
...
MT5_CACHE = DATA_DIR / "mt5_eurusd_h1.parquet"
...
class BridgeEngine:
  ...
  def __init__(
    ...
    *,
    ...
    self._params = get_model_run_params(resolve_model(model_id), model_id)

  @property
  def params(self) -> dict:
    return self._params

  def seed_from_dukascopy(self, force: bool = False) -> pd.DataFrame:
    """Warm-start MT5 series from Dukascopy history if cache empty."""
    if self.mt5_cache.exists() and not force:
      df = pd.read_parquet(self.mt5_cache)
      self._df = _normalize(df)
      return self._df
    duka = load_eurusd_h1()
    self._df = _normalize(duka)
  ...
  def load(self) -> pd.DataFrame:
    ...
    if self.mt5_cache.exists():
      ...
      return self._df
    return self.seed_from_dukascopy()
...
def _parse_bar_time(bar: dict) -> pd.Timestamp:
  ...
  if bar.get("bar_time"):
    return pd.Timestamp(bar["bar_time"])
  if bar.get("time_msc"):
