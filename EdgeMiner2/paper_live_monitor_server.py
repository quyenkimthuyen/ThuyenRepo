...
def _paper_snapshot() -> dict:
  ...
  key = (state.get("updated_at"), cache_mtime)
  with _snapshot_lock:
  ...
  payload = {
    ...
    "connection": {
      ...
      "server_time": state.get("last_bar"),
      "bid": last_close,
      "ask": last_close,
      "bar": last or None,
    },
    "connection_mtime": STATE_PATH.stat().st_mtime if STATE_PATH.exists() else None,
    "trades": list(state.get("orders") or []),
    "state": state,
    "online": bool(state and rows and not state.get("error")),
  }
  with _snapshot_lock:
    _snapshot_key = key
  ...
  class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
      ...
      if parsed.path in ("/", "/chart"):
        ...
        self._send(
          200,
          _chart_html(max_bars, mode="paper").encode("utf-8"),
