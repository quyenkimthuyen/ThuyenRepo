...
def test_eas_use_magic_and_hedging_guards():
  m15 = (M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM5E31.mq5").read_text(encoding="utf-8")
  m15_sim = (M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM5E31Sim.mq5").read_text(encoding="utf-8")
  ...
  assert 'const string INSTANCE_ID = "M5E31"' in m15
  assert 'const string INSTANCE_ID = "M5E31"' in m15_sim
  ...
  assert 'InpBridgeSubdir   = "bridge_m5e31"' in m15
  assert "InpMagic          = 20261021" in m15
  assert 'InpMode = BRIDGE_HISTORY_FEED' in m15_sim
  assert 'InpBridgeSubdir   = "bridge_sim_m5e31"' in m15_sim
  ...
  for root, live, sim in (
    (M15_ROOT, "ForgeBridgeM5E31", "ForgeBridgeM5E31Sim"),
