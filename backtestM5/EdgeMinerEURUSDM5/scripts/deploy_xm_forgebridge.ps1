...
$SourceEaLive = Join-Path $RepoRoot "mt5\Experts\ForgeBridgeM5E31.mq5"
$SourceEaSim = Join-Path $RepoRoot "mt5\Experts\ForgeBridgeM5E31Sim.mq5"
$ProjectBridge = Join-Path $RepoRoot "mt5\bridge_m5e31"
$ProjectBridgeSim = Join-Path $RepoRoot "mt5\bridge_sim_m5e31"
$BridgeSubdirLive = "bridge_m5e31"
$BridgeSubdirSim = "bridge_sim_m5e31"
$EaNameLive = "ForgeBridgeM5E31"
$EaNameSim = "ForgeBridgeM5E31Sim"
$EaFolder = "EdgeMinerEURUSDM5"
$EaMagicLive = 20261061
$EaMagicSim = 20262061
$IsBoth = ($Mode -eq "Both")
...
function New-EurusdM15Chart([string]$DataPath) {
  ...
  if ($templates.Count -eq 0) {
    Write-Host "No EURUSD template anywhere - creating blank EURUSD M5 chart."
  ...
  Write-Host "Created EURUSD M5 chart in active profile: $targetPath"
...
function Get-ForgeFamilyPattern {
  ...
  # ForgeBridgeM5E31\b does not match ForgeBridgeM5E31B4 — must use the open suffix.
...
function Select-AttachChart(
  ...
  if ($free.Count -gt 0) {
    Write-Host "Attach target: free EURUSD M5 chart $($free[0].Name) for $wantedName"
    return $free[0]
  }
  ...
  Write-Host "No free EURUSD M5 chart for $wantedName; creating a new chart (keep $otherName intact)."
  $created = @(New-EurusdM15Chart $DataPath)
  if ($created.Count -eq 0) {
    throw ("Failed to create EURUSD M5 chart for $wantedName.")
  }
...
function Restart-BridgeService {
  ...
  $commandLine = (
    ...
    "--monitor-port 9075 --bridge-dir `"$ProjectBridge`""
  )
...
if ($Attach) {
  if ($IsBoth) {
    ...
    Write-Step "Attach $EaName HISTORY_FEED to EURUSD M5"
    ...
    Write-Step "Attach $EaName Live to EURUSD M5"
