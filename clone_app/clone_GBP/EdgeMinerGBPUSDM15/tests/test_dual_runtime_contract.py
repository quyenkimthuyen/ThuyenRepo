...
def test_eas_use_magic_and_hedging_guards():
  m15 = (M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM15G14.mq5").read_text(encoding="utf-8")
  m15_sim = (M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM15G14Sim.mq5").read_text(encoding="utf-8")
  ...
  assert 'const string INSTANCE_ID = "M15G14"' in m15
  assert 'const string INSTANCE_ID = "M15G14"' in m15_sim
  ...
  assert 'InpBridgeSubdir   = "bridge_m15g14"' in m15
  assert "InpMagic          = 20261014" in m15
  assert 'InpMode = BRIDGE_HISTORY_FEED' in m15_sim
  assert 'InpBridgeSubdir   = "bridge_sim_m15g14"' in m15_sim
  assert "InpMagic          = 20262014" in m15_sim
  ...
  for root, live, sim in (
    (M15_ROOT, "ForgeBridgeM15G14", "ForgeBridgeM15G14Sim"),
