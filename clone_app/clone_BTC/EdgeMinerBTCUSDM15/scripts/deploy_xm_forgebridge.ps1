...
$SourceEaLive = Join-Path $RepoRoot "mt5\Experts\ForgeBridgeM15C13.mq5"
$SourceEaSim = Join-Path $RepoRoot "mt5\Experts\ForgeBridgeM15C13Sim.mq5"
$ProjectBridge = Join-Path $RepoRoot "mt5\bridge_m15c13"
$ProjectBridgeSim = Join-Path $RepoRoot "mt5\bridge_sim_m15c13"
$BridgeSubdirLive = "bridge_m15c13"
$BridgeSubdirSim = "bridge_sim_m15c13"
$EaNameLive = "ForgeBridgeM15C13"
$EaNameSim = "ForgeBridgeM15C13Sim"
$EaFolder = "EdgeMinerM15C13"
$EaMagicLive = 20261013
$EaMagicSim = 20262013
$IsBoth = ($Mode -eq "Both")
...
function Get-BTCUSDM15Charts([string]$DataPath) {
  $chartsRoot = Get-ActiveChartsRoot $DataPath
  $allBTCUSDCharts = Get-ChildItem $chartsRoot -Filter "*.chr" -Recurse |
    Where-Object {
      $text = Get-Content $_.FullName -Raw
      $text -match "(?m)^symbol=BTCUSD\s*$"
    }
  $charts = $allBTCUSDCharts |
    Where-Object {
      $text = Get-Content $_.FullName -Raw
...
function New-MinimalBTCUSDChartText(
  [int]$PeriodType,
  [int]$PeriodSize
) {
...
function Find-BTCUSDChartTemplate([string]$DataPath) {
  ...
  $all = @(Get-ChildItem $chartsBase -Filter "*.chr" -Recurse -ErrorAction SilentlyContinue |
    Where-Object {
      (Get-Content $_.FullName -Raw) -match "(?m)^symbol=BTCUSD\s*$"
    })
...
function New-BTCUSDM15Chart([string]$DataPath) {
  $chartsRoot = Get-ActiveChartsRoot $DataPath
  $family = Get-ForgeFamilyPattern
  ...
  # Active profile empty (e.g. Default with 0 charts) - borrow BTCUSD from any profile.
  if ($templates.Count -eq 0) {
    $borrowed = Find-BTCUSDChartTemplate $DataPath
    if ($borrowed) {
      Write-Host "No BTCUSD in active profile; using template from $($borrowed.Directory.Name)\$($borrowed.Name)"
      $templates = @($borrowed)
    }
  }
  ...
  if ($templates.Count -eq 0) {
    Write-Host "No BTCUSD template anywhere - creating blank BTCUSD M15 chart."
    $text = New-MinimalBTCUSDChartText 0 15
  } else {
  ...
  Write-Host "Created BTCUSD M15 chart in active profile: $targetPath"
...
function Select-AttachChart(
  ...
  $charts = Get-BTCUSDM15Charts $DataPath
  if (-not $charts -or $charts.Count -eq 0) {
    $charts = @(New-BTCUSDM15Chart $DataPath)
  }
  ...
  if ($free.Count -gt 0) {
    Write-Host "Attach target: free BTCUSD M15 chart $($free[0].Name) for $wantedName"
    return $free[0]
  }
  ...
  Write-Host "No free BTCUSD M15 chart for $wantedName; creating a new chart (keep $otherName intact)."
  $created = @(New-BTCUSDM15Chart $DataPath)
  if ($created.Count -eq 0) {
    throw ("Failed to create BTCUSD M15 chart for $wantedName.")
  }
...
function Restart-BridgeService {
  ...
  $commandLine = (
    ...
    "--monitor-port 8895 --bridge-dir `"$ProjectBridge`""
  )
...
if ($Attach) {
  if ($IsBoth) {
    ...
    Write-Step "Attach $EaName HISTORY_FEED to BTCUSD M15"
    ...
    Write-Step "Attach $EaName Live to BTCUSD M15"
