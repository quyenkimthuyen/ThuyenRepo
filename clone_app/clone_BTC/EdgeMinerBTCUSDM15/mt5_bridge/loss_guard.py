...
def evaluate_loss_guard(
  ...
  *,
  ...
  """
  Return a trip payload when consecutive auto losses hit a configured limit.

  ``max_day`` / ``max_week`` <= 0 disables that window.
  """
  cfg = cfg or {}
  ...
  tripped = None
  if max_day > 0 and day_streak >= max_day:
    tripped = {
      "scope": "day",
      "streak": day_streak,
      "limit": max_day,
...
def loss_guard_status(
  ...
  *,
  ...
  """Non-tripping snapshot for UI (streaks + limits)."""
  cfg = cfg or {}
  max_day = int(cfg.get("loss_guard_max_day") or 0)
  max_week = int(cfg.get("loss_guard_max_week") or 0)
...
def build_flat_halt_decision(
  ...
  *,
  ...
  return {
    "action": "FLAT",
    "ok": True,
    "symbol": bar.get("symbol") or "BTCUSD",
