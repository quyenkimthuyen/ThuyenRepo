...
BRIDGE_DIR = ROOT / "mt5" / "bridge_m15c13"
BRIDGE_SIM_DIR = ROOT / "mt5" / "bridge_sim_m15c13"
...
DEFAULT_MODEL_ID = ""
DEFAULT_MAGIC = 20261013
DEFAULT_SIM_MAGIC = 20262013
...
def pip_size_from_quotes(*, digits: int | None = None, point: float | None = None) -> float:
  """Estimate 1 pip in price units (BTCUSD → pip 1.0)."""
...
def prices_from_pips(
  ...
  *,
  ...
  act = action.upper()
  pip = float(pip_size) if pip_size and pip_size > 0 else 1.0
  if act == "BUY":
    entry = float(ask)
