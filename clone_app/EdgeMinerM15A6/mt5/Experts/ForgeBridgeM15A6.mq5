//+------------------------------------------------------------------+
//| ForgeBridgeM15A6.mq5 — EdgeMiner M15A6 (magic 20261006, bridge_m15a6)       |
...
input string InpBridgeSubdir   = "bridge_m15a6";          // under MQL5/Files/ (use bridge_sim for HistoryFeed)
...
int OnInit()
{
   ...
   else if(InpMode == BRIDGE_HISTORY_FEED)
   {
      ...
      {
         Print("ForgeBridgeM15A6 requires a hedging account.");
         return INIT_FAILED;
      }
      g_sim_ea_status = "idle";
      WriteSimControlFile();
      EventSetMillisecondTimer(50);
      Print("ForgeBridgeM15A6 HistoryFeed | Files/", InpBridgeSubdir,
   ...
   else
   {
      ...
      Print("ForgeBridgeM15A6 Live | Files/", InpBridgeSubdir, " | magic=", InpMagic);
   }
...
bool LoadHistoryRatesRange()
{
   ...
   g_open_signal_id = "";
   g_had_position = false;
   Print("ForgeBridgeM15A6 HistoryFeed loaded bars=", g_hist_n,
...
bool PaperOpenFromDecision(const string json, const double entry_price, const string bar_time = "")
{
   ...
   WriteFillJsonEx("open", sid, action, true, "opened",
                   ...
                   false, "strategy", bt);
   Print("ForgeBridgeM15A6 HistoryFeed paper ", action, " @", entry_price,
...
void ProcessHistoryFeed()
{
   ...
   if(g_hist_n < 1 || g_hist_cursor >= g_hist_n)
   {
      ...
      g_sim_enabled = false;
      WriteSimControlFile();
      Print("ForgeBridgeM15A6 HistoryFeed completed bars=", g_hist_n);
      return;
   }
   ...
   if(flat && g_pending_decision == "")
   {
      ...
      if(WaitDecisionForBar(want, json, wait_ms))
      {
         ...
         Print("ForgeBridgeM15A6 HistoryFeed: no decision for ", want,
