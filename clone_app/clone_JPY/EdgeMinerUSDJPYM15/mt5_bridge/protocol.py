...
BRIDGE_DIR = ROOT / "mt5" / "bridge_m15j9"
BRIDGE_SIM_DIR = ROOT / "mt5" / "bridge_sim_m15j9"
...
DEFAULT_MODEL_ID = ""
DEFAULT_MAGIC = 20261009
DEFAULT_SIM_MAGIC = 20262009
...
def pip_size_from_quotes(*, digits: int | None = None, point: float | None = None) -> float:
  """Estimate 1 pip in price units (USDJPY 3-digit → 0.01)."""
...
def prices_from_pips(
  ...
  *,
  ...
  act = action.upper()
  pip = float(pip_size) if pip_size and pip_size > 0 else 0.01
  if act == "BUY":
    entry = float(ask)
