...
$SourceEaLive = Join-Path $RepoRoot "mt5\Experts\ForgeBridgeM15J9.mq5"
$SourceEaSim = Join-Path $RepoRoot "mt5\Experts\ForgeBridgeM15J9Sim.mq5"
$ProjectBridge = Join-Path $RepoRoot "mt5\bridge_m15j9"
$ProjectBridgeSim = Join-Path $RepoRoot "mt5\bridge_sim_m15j9"
$BridgeSubdirLive = "bridge_m15j9"
$BridgeSubdirSim = "bridge_sim_m15j9"
$EaNameLive = "ForgeBridgeM15J9"
$EaNameSim = "ForgeBridgeM15J9Sim"
$EaFolder = "EdgeMinerM15J9"
$EaMagicLive = 20261009
$EaMagicSim = 20262009
$IsBoth = ($Mode -eq "Both")
...
function Get-USDJPYM15Charts([string]$DataPath) {
  $chartsRoot = Get-ActiveChartsRoot $DataPath
  $allUSDJPYCharts = Get-ChildItem $chartsRoot -Filter "*.chr" -Recurse |
    Where-Object {
      $text = Get-Content $_.FullName -Raw
      $text -match "(?m)^symbol=USDJPY\s*$"
    }
  $charts = $allUSDJPYCharts |
    Where-Object {
      $text = Get-Content $_.FullName -Raw
...
function New-MinimalUSDJPYChartText(
  [int]$PeriodType,
  [int]$PeriodSize
) {
...
function Find-USDJPYChartTemplate([string]$DataPath) {
  ...
  $all = @(Get-ChildItem $chartsBase -Filter "*.chr" -Recurse -ErrorAction SilentlyContinue |
    Where-Object {
      (Get-Content $_.FullName -Raw) -match "(?m)^symbol=USDJPY\s*$"
    })
...
function New-USDJPYM15Chart([string]$DataPath) {
  $chartsRoot = Get-ActiveChartsRoot $DataPath
  $family = Get-ForgeFamilyPattern
  ...
  # Active profile empty (e.g. Default with 0 charts) - borrow USDJPY from any profile.
  if ($templates.Count -eq 0) {
    $borrowed = Find-USDJPYChartTemplate $DataPath
    if ($borrowed) {
      Write-Host "No USDJPY in active profile; using template from $($borrowed.Directory.Name)\$($borrowed.Name)"
      $templates = @($borrowed)
    }
  }
  ...
  if ($templates.Count -eq 0) {
    Write-Host "No USDJPY template anywhere - creating blank USDJPY M15 chart."
    $text = New-MinimalUSDJPYChartText 0 15
  } else {
  ...
  Write-Host "Created USDJPY M15 chart in active profile: $targetPath"
...
function Select-AttachChart(
  ...
  $charts = Get-USDJPYM15Charts $DataPath
  if (-not $charts -or $charts.Count -eq 0) {
    $charts = @(New-USDJPYM15Chart $DataPath)
  }
  ...
  if ($free.Count -gt 0) {
    Write-Host "Attach target: free USDJPY M15 chart $($free[0].Name) for $wantedName"
    return $free[0]
  }
  ...
  Write-Host "No free USDJPY M15 chart for $wantedName; creating a new chart (keep $otherName intact)."
  $created = @(New-USDJPYM15Chart $DataPath)
  if ($created.Count -eq 0) {
    throw ("Failed to create USDJPY M15 chart for $wantedName.")
  }
...
function Restart-BridgeService {
  ...
  $commandLine = (
    ...
    "--monitor-port 8855 --bridge-dir `"$ProjectBridge`""
  )
...
if ($Attach) {
  if ($IsBoth) {
    ...
    Write-Step "Attach $EaName HISTORY_FEED to USDJPY M15"
    ...
    Write-Step "Attach $EaName Live to USDJPY M15"
