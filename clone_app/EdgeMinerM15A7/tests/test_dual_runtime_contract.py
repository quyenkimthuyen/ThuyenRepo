...
def test_eas_use_magic_and_hedging_guards():
  m15 = (M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM15A7.mq5").read_text(encoding="utf-8")
  m15_sim = (M15_ROOT / "mt5" / "Experts" / "ForgeBridgeM15A7Sim.mq5").read_text(encoding="utf-8")
  ...
  assert 'const string INSTANCE_ID = "M15A7"' in m15
  assert 'const string INSTANCE_ID = "M15A7"' in m15_sim
  ...
  assert 'InpBridgeSubdir   = "bridge_m15a7"' in m15
  assert "InpMagic          = 20261007" in m15
  assert 'InpMode = BRIDGE_HISTORY_FEED' in m15_sim
  assert 'InpBridgeSubdir   = "bridge_sim_m15a7"' in m15_sim
  assert "InpMagic          = 20262007" in m15_sim
  ...
  for root, live, sim in (
    (M15_ROOT, "ForgeBridgeM15A7", "ForgeBridgeM15A7Sim"),
