...
REOPT_PERIOD_OPTIONS = ("week", "month")
...
def _sanitize_settings(data: dict) -> dict:
  ...
  rp = str(out.get("reopt_period") or "week").lower()
  out["reopt_period"] = rp if rp in REOPT_PERIOD_OPTIONS else "week"
  return out
...
def format_settings_summary(settings: dict | None = None) -> str:
  ...
  reopt = "tuần" if s.get("reopt_period", "week") == "week" else "tháng"
  return (
    f"Học chiến lược: **{trains}** · Cập nhật: **mỗi {reopt}** · "
    f"Giai đoạn học: **{eras}** · Vòng học: **{s.get('learning_loops', 4)}** · "
    f"Kiểm chứng: **{oos}**"
  )
...
def grid_build_kwargs(settings: dict | None = None) -> dict:
  ...
  return {
    ...
    "reopt_period": s.get("reopt_period", "week"),
  }
