...
def render(embedded: bool = False):
  ...
  with c1:
    if st.button("✓ Dùng cho paper & phân tích", key="tm_activate", use_container_width=True):
      ...
      st.rerun()
  with c2:
    if st.button("📦 Export MT5 EA", key="tm_export_mt5", use_container_width=True):
      try:
        from mt5_export.package import export_mt5_package
        out = export_mt5_package(model_id=mid)
        st.success(f"Đã export → `{out}`")
        st.caption("Copy thư mục vào MQL5/Experts/ForexForgeEA/ và compile.")
