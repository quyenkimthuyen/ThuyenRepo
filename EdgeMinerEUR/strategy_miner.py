...
from config import (
  DEFAULT_MAX_HOLD_BARS, MAX_HOLD_GRID,
  DEFAULT_MIN_BARS_BETWEEN, MIN_BARS_BETWEEN_GRID, HYBRID_TRAIL_GRID,
...
class MinedStrategy:
  ...
  min_bars_between: int = DEFAULT_MIN_BARS_BETWEEN
  min_rules_match: int = 2
  max_trades_per_week: int = 2
  max_trades_per_day: int = 0   # >0 → giới hạn theo ngày (ưu tiên hơn tuần)
  ml_prob_min: float = 0.40
  exit_mode: str = "trail"       # full | partial | trail
  partial_pct: float = 0.4
  partial_at_r: float = 1.2
  trail_activate_r: float = DEFAULT_TRAIL_ACTIVATE_R
...
def _label_outcomes(fm, start, end, rr=2.5, atr_mult=0.9, max_hold=None):
  if max_hold is None:
    max_hold = DEFAULT_MAX_HOLD_BARS
  ensure_label_cache_for_df(fm.n)
  key = (fm.n, start, end, rr, atr_mult, max_hold)
  with _LABEL_LOCK:
...
def generate_signals_mined(fm, strat, start_idx=0, end_idx=None):
  ...
  if candidates:
    use_daily = strat.max_trades_per_day > 0
    max_take = strat.max_trades_per_day if use_daily else strat.max_trades_per_week
    if max_take > 0:
      buckets: dict[str, list] = {}
      for score, direction, i in candidates:
        if use_daily:
          bk = fm.index[i].strftime("%Y-%m-%d")
        else:
          bk = fm.index[i].strftime("%Y-%W")
        buckets.setdefault(bk, []).append((score, direction, i))
...
def mine_strategy(fm, train_start, train_end, target_tpw=2.0):
  ...
  exit_modes: list[tuple[str, dict]] = [("full", {})]
  for trail_kw in HYBRID_TRAIL_GRID:
    exit_modes.append(("hybrid", dict(trail_kw)))
  exit_modes.append(("partial", {"partial_pct": 0.35, "partial_at_r": 1.5}))
          ...
          for thr in [0.6, 1.0, 1.6, 2.2]:
            for min_match in [1, 2]:
              for ml_thr in [0.36, 0.40, 0.44, 0.48]:
                for min_gap in MIN_BARS_BETWEEN_GRID:
                  trail_tag = ""
                  if exit_mode == "hybrid":
                    trail_tag = f"_t{exit_kw['trail_activate_r']:.0f}"
                  strat = MinedStrategy(
                    long_rules=long_rules, short_rules=short_rules,
                    score_threshold=thr, atr_mult_sl=atr_m, rr_ratio=rr,
                    min_bars_between=min_gap, min_rules_match=min_match,
                    max_trades_per_week=2, ml_prob_min=ml_thr,
                    max_hold_bars=max_hold,
                    exit_mode=exit_mode, ml_scorer=ml,
                    name=f"v3_{exit_mode}{trail_tag}_rr{rr}_h{max_hold}_g{min_gap}",
                    **exit_kw,
                  )
